...
resource "aws_cloudwatch_metric_alarm" "dhan_wal_dropped" {
  ...
  alarm_description = "A live frame MISSED THE DURABLE FLOOR — the write-ahead log that every zero-loss claim in this system rests on. tv_dhan_ws_wal_dropped_total counts frames the capture-at-receipt WAL refused. READ THE LOG BEFORE ASSUMING TOTAL LOSS: this counter covers TWO outcomes with very different severity, and only the log lines tell them apart. (a) DEGRADED, the common one under a stalled writer — the WAL refused the frame but the in-memory ring took it, so its ticks ARE folded and DO reach the database this session, and only a crash before the next flush loses them. Its line is a WARN reading 'write-ahead log refused a Dhan frame but the ring accepted it', field degraded_on_this_socket. (b) TOTAL LOSS — the WAL refused it AND the ring refused it, so the bytes were never written anywhere and no replay, backfill or cross-verification recovers them. Its line is an ERROR reading 'write-ahead log refused a Dhan frame AND the ring refused it too', field lost_on_this_socket. Both lines are throttled to powers of two, so the FIELD VALUE is the count, not the number of lines. Triage in this order: (1) disk — a full or read-only data volume is the most common cause; check tv_spill_dir_free_bytes and the WS-SPILL-01 alarm, which usually fires first; (2) the WAL writer thread — WS-SPILL-02 means the spill channel was full at the append instant; (3) the coded lines in /tickvault/<env>/app naming the endpoint and pool_index. Frames in case (b) do NOT come back when the disk recovers."
