...
resource "aws_cloudwatch_metric_alarm" "dhan_no_ticks_flowing" {
  ...
  alarm_description = "The Dhan live lane has not PERSISTED a tick for ~10 minutes DURING MARKET HOURS. This is the 'is it actually working' signal: sockets can be connected, the lane-up gauge can read 1, every loss counter can read zero, and still no market data reaches QuestDB - that is what the 2026-08-12 session looked like. Before the first tick of a session the gauge reports the drain's own uptime, so a lane that dials and never receives anything pages instead of reassuring. Missing data breaches deliberately: a dead app publishes nothing at all. Triage in order: (1) tv_dhan_ws_alive_connections and tv_dhan_ws_dial_failed_total - are the sockets up. (2) tv_dhan_feed_drain_frames_total and tv_dhan_feed_ingest_ticks_total - are frames arriving but not folding, then read tv_dhan_feed_ingest_refused_total whose reason label names why. (3) Is QuestDB accepting writes - this gauge is stamped on a flush that PERSISTED rows, so an ILP outage decays it while the socket is busy. (4) journalctl -u tickvault for WS-GAP-03 and the subscribe batching lines. If frames arrive and rows do not persist, the fault is downstream of the socket; if no frames arrive, it is the socket or the subscription."
