...
resource "aws_cloudwatch_metric_alarm" "dhan_wal_dropped" {
  ...
  alarm_description = "A live frame MISSED THE DURABLE FLOOR. tv_dhan_ws_wal_dropped_total counts frames the capture-at-receipt write-ahead log refused. READ THE LOG BEFORE ASSUMING TOTAL LOSS - it covers TWO outcomes and only the log tells them apart. (a) DEGRADED, the common one: the WAL refused the frame but the in-memory ring took it, so its ticks ARE folded and DO reach the database this session; only a crash before the next flush loses them. WARN line, field degraded_on_this_socket. (b) TOTAL LOSS: WAL and ring both refused, so the bytes were never written anywhere and no replay or backfill recovers them. ERROR line, field lost_on_this_socket. Both lines throttle to powers of two, so the FIELD VALUE is the count, not the line count. Triage: (1) disk - full or read-only volume is the usual cause; check tv_spill_dir_free_bytes and WS-SPILL-01; (2) WS-SPILL-02 means the spill channel was full at append; (3) coded lines in /tickvault/<env>/app naming endpoint and pool_index. Case (b) does NOT come back when the disk recovers."
