...
resource "aws_cloudwatch_metric_alarm" "order_audit_chain_loss" {
  ...
  alarm_description = "The SEBI order/position forensic chain LOST A ROW. This one alarm sums five counters that each report a different way an audit row failed to land: rows discarded by the order_audit writer, order_audit persist errors (append or flush), order/position update events dropped before persistence, order_update_events persist errors, and order_update_events rows discarded. SEBI retention is five years and these tables are the only record of what the system did with an order — a lost row is not recoverable from anywhere else, because the broker event that produced it is not replayed. WHAT TO DO: (1) grep /tickvault/<env>/app for the coded ORDER-AUDIT / ORDER-UPDATE lines — they name WHICH of the five mechanisms fired, the stage (append vs flush), and the reason label; (2) a persist_errors breach points at QuestDB (check ILP flush latency and the WAL-suspended-tables gauge) and the rows may still be re-appendable; (3) a rows_discarded or events_dropped breach means the row was rejected or the channel was full and it is already gone — record the window, because a regulator asking about that interval will have a hole in the answer. The order push channels are receive-only PAPER today, so a breach now is the rehearsal failing, not live-order data."
