...
/// Spawn the process-global daily rider. Call ONLY when
/// `[groww_universe] enabled = true` — the config gate is the kill switch and
/// the serde default is OFF.
// TEST-EXEMPT: spawn wrapper over an infinite daily control-plane loop driving
// live network/QuestDB I/O; the pure primitives it composes (secs_until_day_roll,
// is_valid_trading_date, build_and_write_groww_watch's parse/assemble chain,
// persist_groww_instruments' builders) are unit-tested in their own modules.
