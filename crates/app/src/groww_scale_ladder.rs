                        ...
                        let plateau_halt = if smoke {
                            ...
                            match evaluate_plateau(
                                ...
                                PlateauVerdict::Plateau {
                                    ...
                                    gate_breach_streak = 0;
                                    warmup_skip_logged = false;
                                    stage_outcome = Some("halted_at_plateau");
                                    plateau_baseline_proof = None;
                                    plateau_pinned_evals = 0;
                                    plateau_watch_started = None;
                                    Some(next)
...
mod tests {
    ...
    fn test_gate_grace_is_wired_into_runner_loop() {
        ...
        // 6 = the `let mut` init + 4 in-loop re-arms (global halve,
        // rollback, advance, plateau rollback) + this test's own literal
        // (include_str! sees the whole file, tests included).
        assert_eq!(
            src.matches("rung_transition_at = std::time::Instant::now();")
                .count(),
            6,
            "warm-up window must re-arm at all 4 rung transitions \
             (global halve, rollback, advance, plateau rollback)"
        );
        assert!(
