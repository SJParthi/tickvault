...
//! 2. main.rs constructs `OrderSideWiring` on BOTH boot arms (fast
//!    crash-recovery + slow Step 10.5) — a one-arm regression would make
//!    order-side observability boot-path-dependent.
...
#[test]
fn test_main_wires_order_side_wiring_on_both_boot_arms() {
    let src = read_repo_file("src/main.rs");
    let prod = compact(&strip_line_comments(production_region(&src)));
    let constructions = prod.matches("OrderSideWiring{").count();
    assert!(
        constructions >= 2,
        "main.rs must construct OrderSideWiring at BOTH trading-pipeline \
         spawn sites (fast crash-recovery arm + slow Step 10.5); found \
         {constructions} construction(s) — a one-arm regression makes \
         order-side observability boot-path-dependent"
    );
}
