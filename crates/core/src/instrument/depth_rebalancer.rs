...
use super::depth_strike_selector::{
    DEPTH_REBALANCE_STRIKE_THRESHOLD, find_atm_security_ids, should_rebalance,
};
...
pub struct RebalanceEvent {
    ...
    /// New ATM CE security ID (None if chain lookup failed).
    pub new_ce_security_id: Option<u32>,
    /// New ATM PE security ID (None if chain lookup failed).
    pub new_pe_security_id: Option<u32>,
    /// Previous ATM CE security ID (None if first rebalance).
    pub prev_ce_security_id: Option<u32>,
    /// Previous ATM PE security ID (None if first rebalance).
    pub prev_pe_security_id: Option<u32>,
    ...
    loop {
        ...
        for symbol in &underlyings {
            ...
            if should_rebalance(
                ...
                // Look up old/new ATM CE/PE security IDs for the notification.
                let (new_ce, new_pe) = find_atm_security_ids(chain, current_spot);
                let (prev_ce, prev_pe) = find_atm_security_ids(chain, prev_atm);
                ...
                let event = RebalanceEvent {
                    ...
                    new_ce_security_id: new_ce,
                    new_pe_security_id: new_pe,
                    prev_ce_security_id: prev_ce,
                    prev_pe_security_id: prev_pe,
                    expiry: Some(expiry),
                };
...
mod tests {
    ...
    fn test_rebalance_event_debug() {
        let event = RebalanceEvent {
            ...
            drift_strikes: 3,
            new_ce_security_id: Some(35246),
            new_pe_security_id: Some(35247),
            prev_ce_security_id: Some(35100),
            prev_pe_security_id: Some(35101),
