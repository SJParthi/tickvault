...
use tickvault_common::constants::{
    ...
    TICK_PERSIST_END_SECS_OF_DAY_IST, TIMEFRAME_1D, TIMEFRAME_1M,
};
...
// ---------------------------------------------------------------------------
// Phase 0 Item 8+9 PR-D2 (2026-05-17) — Gap-fill REST fetch
// ---------------------------------------------------------------------------
//
// New pub helper for the gap-fill scheduler. Fetches a single 1-minute
// intraday window for ONE (security_id, segment) tuple from Dhan REST
// `/v2/charts/intraday`. Reuses the pub(crate) retry/backoff/classify
// primitives shipped in PR-D1 (#672) so gap-fill follows the SAME
// DH-904 policy as the boot-time historical fetch.
//
// Per the architectural lock in PR-B' (#670):
// - Returns parsed `Vec<HistoricalCandle>` — caller UPSERTs.
// - No `CandlePersistenceWriter` coupling.
// - No state, no side effects beyond the HTTP call.
//
// PR-D3 (next session) wires this fn into `run_gap_fill_scheduler`.

/// Error returned by [`fetch_gap_fill_intraday_window`].
///
/// Variants map onto the existing [`ErrorAction`] classification plus
/// pure-network-failure cases the inner `reqwest::send()` produces.
#[derive(Debug)]
pub enum GapFillFetchError {
    /// Underlying `reqwest` transport error (DNS, connect, TLS, etc.).
    Network(String),
    /// Dhan returned DH-904 / HTTP 429 repeatedly until the
    /// `is_dh904_exhausted` cap was hit. The scheduler should log
    /// `GapFill02RestFetchFailed` and skip this bar.
    RateLimitExhausted { attempts: u32 },
    /// Dhan returned DH-905 (input error). NEVER retry — the caller
    /// likely passed a wrong segment/instrument enum. Fix upstream.
    InputRejected { http_status: u16 },
    /// Dhan returned 807 (token expired). Token refresh is handled by
    /// `TokenManager`; the scheduler should re-fetch after refresh.
    TokenExpired,
    /// Dhan returned 805 (too many connections). The scheduler should
    /// pause for 60s before retrying any gap-fill fetch.
    TooManyConnections,
    /// Response body exceeded [`MAX_RESPONSE_BODY_SIZE`] — a likely
    /// indicator of an upstream bug since 1-minute windows return
    /// O(few-KB) payloads.
    ResponseOversized,
    /// Body failed to parse as a `DhanIntradayResponse`. Carries the
    /// error message for the operator's runbook.
    InvalidResponse(String),
    /// Retry budget exhausted on a 5xx / network-error class without
    /// triggering one of the more specific arms.
    RetryExhausted { attempts: u32 },
}

impl std::fmt::Display for GapFillFetchError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Network(msg) => write!(f, "network error: {msg}"),
            Self::RateLimitExhausted { attempts } => {
                write!(f, "DH-904 rate-limit exhausted after {attempts} attempts")
            }
            Self::InputRejected { http_status } => {
                write!(f, "DH-905 input rejected (HTTP {http_status})")
            }
            Self::TokenExpired => write!(f, "807 token expired — refresh and retry"),
            Self::TooManyConnections => {
                write!(f, "805 too many connections — pause 60s and retry")
            }
            Self::ResponseOversized => write!(f, "response body exceeded max size"),
            Self::InvalidResponse(msg) => write!(f, "invalid response format: {msg}"),
...
impl std::error::Error for GapFillFetchError {}

/// Format an IST epoch-seconds value as `"YYYY-MM-DD HH:MM:SS"` (the
/// `fromDate`/`toDate` format Dhan REST expects).
///
/// Pure function. The value is treated as a Dhan-style IST epoch
/// second per `data-integrity.md` "WebSocket Timestamp Rule" — i.e.
/// the raw seconds-since-1970 in IST wall-clock space.
#[must_use]
pub fn format_dhan_ist_datetime(ist_epoch_secs: i64) -> String {
    use chrono::DateTime;
    // Dhan's interval format = "YYYY-MM-DD HH:MM:SS" with the IST
    // wall-clock numbers. Since our `ist_epoch_secs` is the raw
    // Dhan-style integer (IST midnight = 86400-multiple), formatting
    // as UTC gives the IST string verbatim.
    DateTime::<chrono::Utc>::from_timestamp(ist_epoch_secs, 0).map_or_else(
        || "1970-01-01 00:00:00".to_string(),
        |dt| dt.format("%Y-%m-%d %H:%M:%S").to_string(),
    )
}

/// Convert an `ExchangeSegment` (or its u8 code) to the wire-format
/// segment string Dhan expects in `IntradayRequest::exchange_segment`.
///
/// Returns `None` when the code is the gap-at-6 reserved value or
/// any other unknown — caller should reject the fetch attempt with
/// `GapFillFetchError::InputRejected` before sending the request.
#[must_use]
pub fn dhan_segment_string_from_code(code: u8) -> Option<&'static str> {
    match code {
        0 => Some("IDX_I"),
        1 => Some("NSE_EQ"),
        2 => Some("NSE_FNO"),
        ...
        4 => Some("BSE_EQ"),
        5 => Some("MCX_COMM"),
        // 6 is the documented gap (see annexure-enums.md rule 2)
        7 => Some("BSE_CURRENCY"),
        8 => Some("BSE_FNO"),
        _ => None,
    }
...
/// Construct an `IntradayRequest` for a single gap-fill window.
///
/// Pure function — does not perform I/O. Tested below without the
/// HTTP layer. The async `fetch_gap_fill_intraday_window` calls this
/// internally and posts the result.
///
/// `instrument` is the Dhan instrument-type string per
/// `docs/dhan-ref/05-historical-data.md` — `"EQUITY"`, `"INDEX"`,
/// `"OPTIDX"`, `"FUTIDX"`, `"OPTSTK"`, `"FUTSTK"`, etc.
///
/// Returns `None` if `segment_code` is unknown (e.g. the gap-at-6 enum
/// value); caller MUST treat this as `InputRejected` before sending.
#[must_use]
pub(crate) fn build_gap_fill_intraday_request(
    security_id: u32,
    segment_code: u8,
    instrument: &str,
    from_ist_secs: i64,
    to_ist_secs: i64,
    include_oi: bool,
) -> Option<IntradayRequest> {
    let exchange_segment = dhan_segment_string_from_code(segment_code)?;
    Some(IntradayRequest {
        security_id: security_id.to_string(),
        exchange_segment: exchange_segment.to_string(),
        instrument: instrument.to_string(),
        // Gap-fill always uses 1-minute interval — Dhan's smallest
        // intraday granularity. Bar boundaries align on the minute
        // per `gap_fill_planner.rs` contract.
        interval: "1".to_string(),
        oi: include_oi,
        from_date: format_dhan_ist_datetime(from_ist_secs),
        to_date: format_dhan_ist_datetime(to_ist_secs),
    })
}

/// Fetch a single gap-fill intraday window from Dhan REST.
///
/// On success returns a `Vec<HistoricalCandle>` filtered through
/// `build_valid_intraday_candles` (non-finite/non-positive/inverted-OHLC
/// rows excluded, weekend rows excluded, post-15:30-IST rows excluded).
///
/// Retry policy matches `fetch_intraday_with_retry` for consistency:
/// - 805 (too many connections) → return `TooManyConnections` to caller
///   (caller pauses 60s)
/// - DH-904 / 429 → exponential backoff `[10s, 20s, 40s, 80s]` via
///   `compute_dh904_backoff_secs`; exhaustion returns `RateLimitExhausted`
/// - 807 (token expired) → return immediately (caller refreshes token)
/// - DH-905 (input error) → return immediately (caller fixes upstream)
/// - Other → linear backoff per `compute_retry_delay_ms`
///
/// `max_retries` follows the existing `HistoricalDataConfig.max_retries`
/// contract. PR-D3 wires this constant from config; PR-D2 ships the
/// function with retry support but the caller currently controls the
/// budget.
///
/// # Test coverage
///
/// Pure-function inputs are covered by inline unit tests
/// (`build_gap_fill_intraday_request`, `format_dhan_ist_datetime`,
/// `dhan_segment_string_from_code`). The async HTTP path is covered
/// by the scheduler integration in PR-D3 (the call site that wires
/// this fn to a live `reqwest::Client`).
// TEST-EXEMPT: async HTTP path integration-tested in PR-D3 via the scheduler wiring; pure helpers (request builder, datetime formatter, segment mapper) covered by unit tests in this module
pub async fn fetch_gap_fill_intraday_window(
    http_client: &reqwest::Client,
    endpoint: &str,
    access_token: &Zeroizing<String>,
    client_id: &SecretString,
    ...
    include_oi: bool,
    max_retries: u32,
) -> Result<Vec<HistoricalCandle>, GapFillFetchError> {
    let request_body = build_gap_fill_intraday_request(
        security_id,
        segment_code,
        instrument,
        from_ist_secs,
    ...
    .ok_or_else(|| GapFillFetchError::InputRejected {
        ...
        match result {
            Ok(response) => {
                if !response.status().is_success() {
                    let status = response.status();
                    let err_body = response.bytes().await.unwrap_or_default();
                    match classify_error(status, &err_body) {
                        ErrorAction::TooManyConnections => {
                            return Err(GapFillFetchError::TooManyConnections);
                        }
                        ErrorAction::RateLimited => {
                            if is_dh904_exhausted(attempt) {
                        ...
                        ErrorAction::TokenExpired => {
                            return Err(GapFillFetchError::TokenExpired);
                        }
                        ErrorAction::NeverRetry => {
                            return Err(GapFillFetchError::InputRejected {
                ...
                let body_bytes = response.bytes().await.map_err(|err| {
                    GapFillFetchError::Network(format!("body read failed: {err}"))
                })?;

                if is_response_oversized(body_bytes.len()) {
                    return Err(GapFillFetchError::ResponseOversized);
                }

                let parsed: DhanIntradayResponse = serde_json::from_slice(&body_bytes)
                    .map_err(|err| GapFillFetchError::InvalidResponse(err.to_string()))?;

                let (candles, _invalid_count) =
                    build_valid_intraday_candles(&parsed, security_id, segment_code, TIMEFRAME_1M);
                return Ok(candles);
            }
            Err(err) => {
                if attempt == max_retries {
                    return Err(GapFillFetchError::Network(err.to_string()));
                }
                // Otherwise loop; linear backoff applies on next iteration.
            }
        }
    }
    Err(GapFillFetchError::RetryExhausted {
...
mod tests {
    ...
    // -----------------------------------------------------------------
    // Phase 0 Item 8+9 PR-D2 (2026-05-17) — gap-fill REST fetch tests
    // -----------------------------------------------------------------

    #[test]
    fn test_format_dhan_ist_datetime_unix_epoch_zero() {
        // IST epoch 0 = "1970-01-01 00:00:00" by Dhan convention
        // (raw integer interpreted directly as IST wall-clock).
        assert_eq!(format_dhan_ist_datetime(0), "1970-01-01 00:00:00");
    }

    #[test]
    fn test_format_dhan_ist_datetime_one_minute_after_epoch() {
        assert_eq!(format_dhan_ist_datetime(60), "1970-01-01 00:01:00");
    }

    #[test]
    fn test_format_dhan_ist_datetime_one_day_after_epoch() {
        // 86400 secs = full day → next IST midnight.
        assert_eq!(format_dhan_ist_datetime(86_400), "1970-01-02 00:00:00");
    }

    #[test]
    fn test_format_dhan_ist_datetime_typical_trading_minute() {
        // 2026-05-17 11:30:00 IST = a typical mid-session minute.
        // Using `naive.and_utc().timestamp()` per the same convention
        // as the gap_fill_scheduler tests (IST midnight is a multiple
        // of 86400 under Dhan's raw-integer convention).
        ...
        assert_eq!(format_dhan_ist_datetime(secs), "2026-05-17 11:30:00");
    }

    #[test]
    fn test_dhan_segment_string_from_code_known_segments() {
        // Annexure-enums.md rule 2: IDX_I=0, NSE_EQ=1, NSE_FNO=2,
        // NSE_CURRENCY=3, BSE_EQ=4, MCX_COMM=5, BSE_CURRENCY=7,
        // BSE_FNO=8. Gap at 6 returns None.
        assert_eq!(dhan_segment_string_from_code(0), Some("IDX_I"));
        assert_eq!(dhan_segment_string_from_code(1), Some("NSE_EQ"));
        assert_eq!(dhan_segment_string_from_code(2), Some("NSE_FNO"));
        assert_eq!(dhan_segment_string_from_code(3), Some("NSE_CURRENCY"));
        assert_eq!(dhan_segment_string_from_code(4), Some("BSE_EQ"));
        assert_eq!(dhan_segment_string_from_code(5), Some("MCX_COMM"));
        assert_eq!(dhan_segment_string_from_code(7), Some("BSE_CURRENCY"));
        assert_eq!(dhan_segment_string_from_code(8), Some("BSE_FNO"));
    }

    #[test]
    fn test_dhan_segment_string_from_code_gap_at_six_returns_none() {
        // Per annexure-enums.md rule 2 and `from_byte()` contract:
        // 6 is the documented gap. Returns None so callers reject the
        // fetch with `GapFillFetchError::InputRejected` instead of
        // sending a request with an unknown segment string.
        assert_eq!(dhan_segment_string_from_code(6), None);
    }

    #[test]
    fn test_dhan_segment_string_from_code_unknown_returns_none() {
        // 9..255 are not assigned in the annexure. All return None.
        assert_eq!(dhan_segment_string_from_code(9), None);
        assert_eq!(dhan_segment_string_from_code(100), None);
        assert_eq!(dhan_segment_string_from_code(255), None);
    }

    #[test]
    fn test_build_gap_fill_intraday_request_nifty_idx_i_one_minute() {
        // NIFTY (security_id=13, IDX_I=0) for a 1-minute window:
        // 2026-05-17 11:30:00 → 11:31:00 IST.
        use chrono::{NaiveDate, NaiveTime};
        let naive = chrono::NaiveDateTime::new(
            NaiveDate::from_ymd_opt(2026, 5, 17).unwrap(),
            NaiveTime::from_hms_opt(11, 30, 0).unwrap(),
        );
        let from = naive.and_utc().timestamp();
        let to = from + 60;
        let req = build_gap_fill_intraday_request(13, 0, "INDEX", from, to, false)
            .expect("known segment must build a request");
        assert_eq!(req.security_id, "13");
        assert_eq!(req.exchange_segment, "IDX_I");
        assert_eq!(req.instrument, "INDEX");
        assert_eq!(req.interval, "1");
        assert!(!req.oi);
        assert_eq!(req.from_date, "2026-05-17 11:30:00");
        assert_eq!(req.to_date, "2026-05-17 11:31:00");
    }

    #[test]
    fn test_build_gap_fill_intraday_request_unknown_segment_returns_none() {
        // Gap-at-6 must short-circuit BEFORE constructing the request.
        let req = build_gap_fill_intraday_request(13, 6, "INDEX", 100, 160, false);
        assert!(req.is_none(), "gap-at-6 must return None pre-flight");
    }

    #[test]
    fn test_build_gap_fill_intraday_request_includes_oi_when_requested() {
        // F&O contracts request OI; equity does not.
        let req = build_gap_fill_intraday_request(49081, 2, "OPTIDX", 100, 160, true)
            .expect("NSE_FNO segment must build");
        assert!(req.oi, "include_oi=true must propagate to request");
        assert_eq!(req.exchange_segment, "NSE_FNO");
        assert_eq!(req.instrument, "OPTIDX");
    }

    #[test]
    fn test_gap_fill_fetch_error_display_includes_diagnostic_context() {
        // Operator runbook quality: error messages should be
        // self-describing without requiring source-code reference.
        let err = GapFillFetchError::RateLimitExhausted { attempts: 5 };
        let msg = format!("{err}");
        assert!(msg.contains("DH-904"));
        assert!(msg.contains("5"));

        let err = GapFillFetchError::TokenExpired;
        let msg = format!("{err}");
        assert!(msg.to_lowercase().contains("token"));
        assert!(msg.to_lowercase().contains("refresh"));

        let err = GapFillFetchError::TooManyConnections;
        let msg = format!("{err}");
        assert!(msg.contains("805"));
        assert!(msg.to_lowercase().contains("60s"));

        let err = GapFillFetchError::InputRejected { http_status: 400 };
        let msg = format!("{err}");
        assert!(msg.contains("DH-905"));
        assert!(msg.contains("400"));
    }

    #[test]
    fn test_gap_fill_fetch_error_is_send_sync_static() {
        // The gap-fill scheduler will store this error type in
        // tracing spans + Telegram event payloads, both of which
        // require Send + Sync + 'static.
        fn assert_send_sync_static<T: Send + Sync + 'static>() {}
        assert_send_sync_static::<GapFillFetchError>();
    }
}
