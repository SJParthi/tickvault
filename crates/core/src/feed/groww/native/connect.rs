//! NATS `INFO` → `CONNECT` handshake pieces for the Groww native shadow
//! client — the two pure halves of the auth exchange:
//!
//! 1. [`extract_nonce`] — pull the signing `nonce` out of the server `INFO`
//!    JSON (delivered by [`crate::feed::groww::nats::NatsServerOp::Info`]).
...
//! ([`crate::feed::groww::nats::parse_frame`]) intentionally does not speak
//! `HMSG`.
//!
