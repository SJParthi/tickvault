...
use dhan_live_trader_common::tick_types::{ParsedTick, PrevCloseRecord};
...
/// * `prev_close_sender` — optional channel for forwarding PrevClose to cold-path consumer
///   (used in fast boot when tick_writer is None)
#[allow(clippy::too_many_arguments)] // APPROVED: tick processor orchestrates all hot-path outputs
pub async fn run_tick_processor(
    ...
    prev_close_sender: Option<mpsc::Sender<PrevCloseRecord>>,
) {
    ...
    while let Some(raw_frame) = frame_receiver.recv().await {
        ...
        match parsed {
            ...
            ParsedFrame::PreviousClose {
                ...
                // Persist previous close to QuestDB (direct write if writer available,
                // or forward to cold-path consumer via channel in fast boot mode).
                if let Some(ref mut writer) = tick_writer {
                ...
                } else if let Some(ref pcs) = prev_close_sender {
                    // Fast boot: tick_writer is None, forward to cold-path consumer.
                    // try_send is O(1) non-blocking — never stalls the hot path.
                    let _ = pcs.try_send(PrevCloseRecord {
                        security_id,
                        exchange_segment_code,
                        previous_close,
                        previous_oi,
