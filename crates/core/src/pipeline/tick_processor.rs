...
mod tests {
    ...
    async fn test_tick_processor_processes_frames() {
        ...
        let handle = tokio::spawn(async move {
            run_tick_processor(frame_rx, None, None, None).await;
        });
    ...
    async fn test_tick_processor_with_writer_appends_valid_ticks() {
        ...
        let handle = tokio::spawn(async move {
            run_tick_processor(frame_rx, Some(writer), None, None).await;
        });
    ...
    async fn test_tick_processor_with_depth_writer_persists_full_quote_depth() {
        ...
        let (frame_tx, frame_rx) = mpsc::channel(100);
        let handle = tokio::spawn(async move {
            run_tick_processor(frame_rx, Some(tick_writer), Some(depth_writer), None).await;
        });
