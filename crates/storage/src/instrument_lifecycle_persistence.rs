        ...
        .build()
    {
        ...
        Err(err) => {
            error!(
                ...
                "HTTP-CLIENT-01 reqwest client build failed — instrument_lifecycle DDL \
        ...
        .build()
    {
        ...
        Err(err) => {
            error!(
                error = %err,
                code = tickvault_common::error_code::ErrorCode::HttpClient01BuildFailed.code_str(),
                "HTTP-CLIENT-01 reqwest client build failed — instrument_lifecycle_audit DDL \
                 skipped: if the table does not exist yet, ILP auto-create will lack DEDUP \
                 keys until the next successful boot (duplicate-row window)"
            );
            metrics::counter!(
                "tv_http_client_build_failed_total",
                "site" => "lifecycle_audit_ensure"
            )
            .increment(1);
            return;
        }
...
// ============================================================================
// C4 sweep (2026-07-15): the O(1) WARM-BOOT primitives
// (`build_bump_active_last_seen_sql` + `bump_active_last_seen`) were DELETED
...
// sweep). The `instrument_lifecycle` SEBI table and every other persistence
// fn stay.
...
mod tests {
    ...
    #[tokio::test]
    async fn test_update_state_mock_200_and_500() {
        // C4 sweep (2026-07-15): the bump_active_last_seen halves died with
        // the deleted warm-boot primitives; the state-update halves stay.
        let ok_port = p2c_spawn_mock_http(P2C_HTTP_200).await;
