//! Human-readable console views and candle-derived Top Volume history.
//!
//! Primary `top_volume_<tf>` views read the corresponding candle table's
//! signed whole-bar volume and pinned metadata. The four older timer-based gross
//! views remain available as `top_volume_legacy_<cadence>`; their physical
//! history is retained separately and never relabeled as candle signed flow.
//!
...
#[path = "candle_top_volume_views.rs"]
mod candle_top_volume_views;
pub use candle_top_volume_views::{
    CANDLE_TOP_VOLUME_METRIC_VERSION, candle_top_volume_view_ddl, candle_top_volume_view_name,
};
...
/// Live 1m candle base table from the same registry used by its producer.
pub const NAMED_VIEW_CANDLES_BASE: &str = TfIndex::M1.table_name();
...
/// Historical gross-activity snapshot view name. The original physical
/// `top_volume` table is retained; its four timer-based views now carry an
/// explicit legacy prefix so they cannot be confused with candle signed flow.
#[must_use]
pub fn top_volume_legacy_view_name(cadence: SnapshotCadence) -> String {
    format!("top_volume_legacy_{}", cadence.as_str())
}

/// DDL for retained timer-based gross-volume history. Primary `top_volume_<tf>`
/// names are now generated by [`candle_top_volume_view_ddl`] from candles.
///
/// These legacy rows preserve their original stored lot size, rounded display
/// values and timer boundary. Their percentage is gross activity relative to
/// one lot, never a candle's signed net quantity. Exact ratio order still uses
/// the stored positive delta/lot pair; underlying price is display context.
pub fn top_volume_legacy_cadence_view_ddl(cadence: SnapshotCadence) -> String {
    let view = top_volume_legacy_view_name(cadence);
    let tf = cadence.as_str();
    let dim = lifecycle_dim_subquery();
    let exact_order = top_volume_exact_ratio_order_sql();
    format!(
        "CREATE OR REPLACE VIEW {view} AS \
         ...
         'gross_activity_vs_one_lot_v1' AS metric_version, \
         'legacy_timer_counter_delta' AS quantity_basis, \
         'stored_legacy_snapshot' AS lot_metadata_basis \
         ...
         AND t.feed = il.feed \
         WHERE t.tf = '{tf}' \
         ORDER BY t.ts DESC, t.family ASC, {exact_order}, \
         t.security_id ASC, \
         CASE WHEN t.segment = 'IDX_I' THEN 0 \
              WHEN t.segment = 'NSE_EQ' THEN 1 \
              WHEN t.segment = 'NSE_FNO' THEN 2 \
              WHEN t.segment = 'NSE_CURRENCY' THEN 3 \
              WHEN t.segment = 'BSE_EQ' THEN 4 \
              WHEN t.segment = 'MCX_COMM' THEN 5 \
              WHEN t.segment = 'BSE_CURRENCY' THEN 6 \
              WHEN t.segment = 'BSE_FNO' THEN 7 \
              ELSE 255 END ASC, t.segment ASC, t.feed ASC;"
    )
}

/// Exact rational ordering for the producer's positive u32 numerator and
/// denominator domain, using only signed LONG arithmetic in QuestDB.
///
/// Integer part plus 64 fraction bits distinguish any unequal u32-denominator
/// ratios: their separation is at least 1 / (d1 * d2), greater than 2^-64.
/// Generate fraction bits in 31 + 31 + 2 bit limbs so every remainder product
/// is below 2^63. Repeating the expressions costs query work, outside the RAM
/// decision path. No extra stored columns or floating sort keys are needed.
///
/// Invalid/legacy rows remain visible after valid rows in their own timestamp
/// and family group. Safe substitute operands avoid division by zero even if
/// the SQL planner evaluates a sort expression before its domain flag.
/// Live QuestDB execution is a release gate; see its documented integer `/`
/// and `%` semantics: <https://questdb.com/docs/query/operators/numeric/>.
fn top_volume_exact_ratio_order_sql() -> String {
    let valid = "t.delta_units BETWEEN 1 AND 4294967295 AND t.lot_size BETWEEN 1 AND 4294967295";
    let numerator = format!("(CASE WHEN {valid} THEN t.delta_units ELSE cast(0 AS LONG) END)");
    let denominator = format!("(CASE WHEN {valid} THEN t.lot_size ELSE cast(1 AS LONG) END)");
    ...
    format!(
        "CASE WHEN {valid} THEN 1 ELSE 0 END DESC, \
...
/// GET-only (POST 405s per the #T1a regression note). A successful HTTP status
/// alone is insufficient: the response must acknowledge `ddl: "OK"` and carry
/// no SQL error. The returned readiness lets mandatory views gate startup.
async fn run_view_ddl(
    client: &Client,
    base_url: &str,
    view: &str,
    action: &str,
    ...
    match client.get(base_url).query(&[("query", ddl)]).send().await {
        Ok(resp) if resp.status().is_success() => {
            let accepted = resp.json::<serde_json::Value>().await.is_ok_and(|body| {
                body.get("ddl").and_then(serde_json::Value::as_str) == Some("OK")
            ...
            if accepted {
                ...
                metrics::counter!(VIEW_DDL_COUNTER, "outcome" => "invalid_response").increment(1);
                warn!(
                    view,
                    action, "named view DDL response did not confirm acceptance"
                );
            }
            accepted
        }
...
/// `non_2xx`, `transport`, `invalid_response`).
///
...
pub const VIEW_DDL_OUTCOMES: [&str; 4] = ["ok", "non_2xx", "transport", "invalid_response"];
...
pub async fn ensure_named_views(questdb_config: &QuestDbConfig) -> bool {
    pre_register_view_ddl_counter();
    // The lifecycle dimension must exist before CREATE VIEW validates its
    // joins. Any failure is reflected by the mandatory projection DDL below.
    ...
    // A failed client cannot establish the mandatory projection contract.
        ...
        Err(err) => {
            error!(
                ...
                "HTTP-CLIENT-01 reqwest client build failed — named-view DDL skipped: \
                 required Top Volume projections could not be established; candle \
                 startup must refuse this unconfirmed schema"
            );
            metrics::counter!(
    ...
    // Preserve the timer-derived record under explicit legacy names before
    // converging primary names to candle-derived quantities. No table history
...
// Try every selected frame so a refusal cannot hide the remaining outcomes.
async fn ensure_primary_candle_views(client: &Client, base_url: &str) -> bool {
    let mut primary_ready = true;
    // The selected Top Volume registry drives the four primary views. All ten
    // physical candle tables remain owned by the separate candle registry.
    // Candle metadata ALTERs must complete before these definitions can be
    // accepted. A rejected DDL remains visible through the outcome counter;
    // deployment readiness must require all primary definitions to converge.
    for tf in TfIndex::TOP_VOLUME_ALL {
        let (Some(view), Some(ddl)) = (
            candle_top_volume_view_name(tf),
            candle_top_volume_view_ddl(tf),
        ) else {
            warn!(
                ?tf,
                "unsupported primary Top Volume timeframe refused before DDL"
            );
            primary_ready = false;
            continue;
        };
        primary_ready &=
            run_view_ddl(client, base_url, &view, "create_candle_projection", &ddl).await;
    }
    primary_ready
}
...
mod tests {
    ...
    async fn ddl_server(
        ...
        let worker = tokio::spawn(async move {
            ...
            for (status, body) in responses {
                ...
                loop {
                    ...
                    assert!(request.len() <= 131_072, "bounded fixture request");
        ...
        (format!("http://{address}/exec"), worker)
    }

    #[tokio::test]
    async fn primary_projection_readiness_requires_all_four_http_acceptances() {
        let client = Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        for failed_index in 0..=TfIndex::TOP_VOLUME_ALL.len() {
            let responses = (0..TfIndex::TOP_VOLUME_ALL.len())
                .map(|index| {
                    if index == failed_index {
                        (500, "{\"error\":\"fixture refused\"}")
                    } else {
                        (200, "{\"ddl\":\"OK\"}")
                    }
                })
            ...
            let accepted = ensure_primary_candle_views(&client, &endpoint).await;
            assert_eq!(accepted, failed_index == TfIndex::TOP_VOLUME_ALL.len());
            ...
            assert_eq!(
                ...
                "one refusal must not skip remaining frames"
            );
            for tf in TfIndex::TOP_VOLUME_ALL {
                let view = candle_top_volume_view_name(tf).unwrap();
                assert_eq!(
                    queries
                        .iter()
                        .filter(|query| query
    ...
    #[tokio::test]
    async fn successful_http_without_a_ddl_acknowledgement_is_not_projection_readiness() {
        let client = Client::builder()
            .timeout(Duration::from_secs(2))
            .build()
            .unwrap();
        for body in [
            ...
            "not json",
            "{\"error\":\"invalid SQL\"}",
            "{\"ddl\":\"OK\",\"error\":\"refused\"}",
        ] {
            let (endpoint, server) = ddl_server(vec![(200, body)]).await;
            assert!(
                !run_view_ddl(
                    &client,
                    &endpoint,
                    "fixture",
                    "create",
                    "CREATE VIEW fixture AS SELECT 1"
                )
                .await
            );
    ...
    #[tokio::test]
    async fn unavailable_http_cannot_confirm_primary_projections() {
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let address = listener.local_addr().unwrap();
        drop(listener);
        let client = Client::builder()
            ...
            .build()
            .unwrap();
        assert!(!ensure_primary_candle_views(&client, &format!("http://{address}/exec")).await);
    }

    // Every generated surface participates in the common SQL-shape checks:
    // three named views, four legacy cadences and four selected Top Volume candle frames.
    fn both_ddls() -> Vec<(String, String)> {
        let mut ddls: Vec<(String, String)> = vec![
            ("ticks_named".to_owned(), ticks_named_view_ddl()),
            ("candles_named".to_owned(), candles_named_view_ddl()),
            ("market_depth_named".to_owned(), depth_named_view_ddl()),
        ];
        for cadence in SnapshotCadence::ALL {
            ddls.push((
                top_volume_legacy_view_name(cadence),
        ...
        for tf in TfIndex::TOP_VOLUME_ALL {
            ddls.push((
                candle_top_volume_view_name(tf).expect("selected primary view"),
                candle_top_volume_view_ddl(tf).expect("selected primary DDL"),
            ));
        }
        ddls
    }
    ...
    #[test]
    fn top_volume_legacy_view_name_and_ddl_preserve_the_old_metric_explicitly() {
        for cadence in SnapshotCadence::ALL {
            let name = top_volume_legacy_view_name(cadence);
            assert_eq!(name, format!("top_volume_legacy_{}", cadence.as_str()));
            let ddl = top_volume_legacy_cadence_view_ddl(cadence);
            assert!(ddl.starts_with(&format!("CREATE OR REPLACE VIEW {name} AS ")));
            assert!(ddl.contains("FROM top_volume t "));
            assert!(ddl.contains("'gross_activity_vs_one_lot_v1' AS metric_version"));
            assert!(ddl.contains("'legacy_timer_counter_delta' AS quantity_basis"));
            for tf in TfIndex::TOP_VOLUME_ALL {
                assert_ne!(
                    name,
                    candle_top_volume_view_name(tf).expect("selected primary view")
    ...
    #[test]
    fn all_generated_view_names_are_unique_and_all_registries_are_covered() {
        let ddls = both_ddls();
        assert_eq!(
            ddls.len(),
            3 + SnapshotCadence::ALL.len() + TfIndex::TOP_VOLUME_ALL.len()
        );
        let names: std::collections::BTreeSet<_> = ddls.iter().map(|(name, _)| name).collect();
        assert_eq!(names.len(), ddls.len());
    }
    ...
    #[test]
    fn test_top_volume_legacy_cadence_view_ddl_filters_on_its_own_stored_cadence() {
        ...
        for c in SnapshotCadence::ALL {
            ...
            let ddl = top_volume_legacy_cadence_view_ddl(c);
    ...
    #[test]
    fn every_top_volume_cadence_orders_snapshot_family_then_volume_not_price_gain() {
        use tickvault_common::types::ExchangeSegment;

        for cadence in SnapshotCadence::ALL {
            let ddl = top_volume_legacy_cadence_view_ddl(cadence);
            let (projection, ordering) = ddl.split_once(" ORDER BY ").unwrap();
            assert!(projection.contains(&format!("WHERE t.tf = '{}'", cadence.as_str())));
            assert!(ordering.starts_with(&format!(
                "t.ts DESC, t.family ASC, {}, t.security_id ASC, CASE ",
                top_volume_exact_ratio_order_sql()
            )));
            assert!(!ordering.contains("gain_pct"));
            assert!(!ordering.contains("window_lots"));
            assert!(!ordering.contains("net_volume_chg_milli_pct"));
            assert!(!ordering.contains("DOUBLE"));
            assert!(
                !ddl.contains("LIMIT"),
                "view must retain the full population"
            );
            // Wire codes differ from ordinals for the final two segments.
            // Derive expected ties from the same Rust enum used by ranking.
            for segment in [
                ...
                assert!(ordering.contains(&format!(
                    "WHEN t.segment = '{}' THEN {} ",
                    segment.as_str(),
                    segment as u8
                )));
            }
            assert!(ordering.ends_with("ELSE 255 END ASC, t.segment ASC, t.feed ASC;"));
