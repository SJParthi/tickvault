...
pub enum ErrorCode {
    ...
    /// CADENCE-05: the degraded-leg RECOVERY machinery itself degraded —
    /// every bounded late retry stayed empty and the decision fell to
    /// cross-fill (`stage="retry_still_empty"`, coalesced per lane per
    /// cycle), a retry attempt hit a 429 and the remaining micro-retries
    /// aborted (`stage="retry_rate_limited"`, coalesced), or the
    /// background history re-pull exhausted its bounded attempts without
    /// rows (`stage="repull_exhausted"`). Per-attempt retry errors are
    /// counter-only (`tv_cadence_late_retry_total{outcome="error"}`) —
    /// the underlying failure already logs via the CADENCE-01 fetch
    /// paths. Observability only — nothing is auto-re-fired;
    /// log-sink-only (operator 2026-07-20). Severity::High,
    /// auto-triage-safe.
...
impl ErrorCode {
    ...
    pub const fn severity(self) -> Severity {
        match self {
            ...
            // CADENCE-05 (operator 2026-07-20): the degraded-leg recovery
            ...
            Self::Cadence05RecoveryDegraded => Severity::High,
...
mod tests {
    ...
    fn test_all_list_length_matches_catalogue_size() {
        ...
        // +1 CADENCE-05 (Cadence05RecoveryDegraded — hedged late-retry /
    ...
    fn test_cadence_codes_contract() {
        ...
        assert_eq!(c5.severity(), Severity::High);
        // Observability only — nothing is auto-re-fired (operator
        // 2026-07-20); log-sink-only per cadence-error-codes.md §CADENCE-05.
