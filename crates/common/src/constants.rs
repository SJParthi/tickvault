...
// Arity guard: the Groww leg stays LOCKED at the 3 §38-granted indices —
// the 2026-07-13 INDIA VIX scope addition is Dhan-spot-only (relayed via
// the coordinator session), so the Dhan set is now 4 while Groww remains 3
// (a 4th Groww index needs a fresh dated §38 quote first). The chain-scope
// subset [`CHAIN_1M_UNDERLYINGS`] is the matching 3-index set.
const _: () = assert!(
    GROWW_SPOT_1M_SYMBOLS.len() == 3 && GROWW_SPOT_1M_SYMBOLS.len() == CHAIN_1M_UNDERLYINGS.len(),
    "GROWW_SPOT_1M_SYMBOLS must stay the 3 §38-granted indices (VIX is Dhan-spot-only)"
);
...
mod tests {
    ...
    fn test_groww_spot_1m_constants_pinned() {
        ...
        // Groww stays LOCKED at the 3 §38-granted indices; the Dhan set is
        // 4 since the 2026-07-13 INDIA VIX spot-only scope addition (VIX is
        // Dhan-spot-only — a 4th Groww index needs a fresh dated §38 quote).
