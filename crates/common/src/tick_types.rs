...
pub struct ParsedTick {
    ...
    /// Exchange timestamp as IST epoch seconds from Dhan V2 API (stored as-is).
...
/// Timestamps are IST epoch seconds from Dhan V2 REST API (stored as-is).
#[derive(Debug, Clone, Copy)]
pub struct HistoricalCandle {
    /// Candle open timestamp as IST epoch seconds (stored as-is from Dhan).
...
/// Dhan V2 REST API returns timestamps as IST epoch seconds — the epoch value
/// represents IST wall-clock time, NOT UTC. Stored as-is in QuestDB.
///
...
pub struct DhanIntradayResponse {
    ...
    pub volume: Vec<i64>,
    /// Timestamps as IST epoch seconds from Dhan V2 REST API.
