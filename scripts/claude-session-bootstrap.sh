...
# Probe a profile and echo `<reachable_count> <prom_s> <alert_s> <qdb_s> <api_s>`.
# Side effects: sets global vars PROM, ALERT, QDB, API, LOGS_SRC, LOGS_DIR
...
score_profile() {
    ...
    REACHABLE_COUNT=0
    for s in "$PROM_S" "$ALERT_S" "$QDB_S" "$API_S"; do
...
if [ "$AUTO_SWITCH_ENABLED" = "1" ] && [ "$REACHABLE_COUNT" -lt "$AUTO_SWITCH_MIN_REACHABLE" ]; then
    ...
    BEST_PROM="$PROM" BEST_ALERT="$ALERT" BEST_QDB="$QDB" BEST_API="$API"
    BEST_LOGS_SRC="$LOGS_SRC" BEST_LOGS_DIR="$LOGS_DIR"
    BEST_PROM_S="$PROM_S" BEST_ALERT_S="$ALERT_S" BEST_QDB_S="$QDB_S" BEST_API_S="$API_S"
    ...
    if [ "$BEST_PROFILE" != "$PROFILE" ]; then
        ...
        PROM="$BEST_PROM" ALERT="$BEST_ALERT" QDB="$BEST_QDB" API="$BEST_API"
        LOGS_SRC="$BEST_LOGS_SRC" LOGS_DIR="$BEST_LOGS_DIR"
        PROM_S="$BEST_PROM_S" ALERT_S="$BEST_ALERT_S" QDB_S="$BEST_QDB_S" API_S="$BEST_API_S"
...
# switched_from: ${AUTO_SWITCHED_FROM:-none}    reachable: $REACHABLE_COUNT/4
...
if [ -n "$AUTO_SWITCHED_FROM" ]; then
    echo "bootstrap: profile=$PROFILE (auto-switched from $AUTO_SWITCHED_FROM, reachable=$REACHABLE_COUNT/4)" >&2
else
