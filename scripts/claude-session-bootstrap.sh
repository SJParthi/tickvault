...
# Probe a profile and echo `<reachable_count> <prom_s> <qdb_s> <api_s>`.
# Side effects: sets global vars PROM, QDB, API, LOGS_SRC, LOGS_DIR
...
score_profile() {
    ...
    REACHABLE_COUNT=0
    for s in "$PROM_S" "$QDB_S" "$API_S"; do
...
if [ "$AUTO_SWITCH_ENABLED" = "1" ] && [ "$REACHABLE_COUNT" -lt "$AUTO_SWITCH_MIN_REACHABLE" ]; then
    ...
    BEST_PROM="$PROM" BEST_QDB="$QDB" BEST_API="$API"
    BEST_LOGS_SRC="$LOGS_SRC" BEST_LOGS_DIR="$LOGS_DIR"
    BEST_PROM_S="$PROM_S" BEST_QDB_S="$QDB_S" BEST_API_S="$API_S"
    ...
    if [ "$BEST_PROFILE" != "$PROFILE" ]; then
        ...
        PROM="$BEST_PROM" QDB="$BEST_QDB" API="$BEST_API"
        LOGS_SRC="$BEST_LOGS_SRC" LOGS_DIR="$BEST_LOGS_DIR"
        PROM_S="$BEST_PROM_S" QDB_S="$BEST_QDB_S" API_S="$BEST_API_S"
...
# switched_from: ${AUTO_SWITCHED_FROM:-none}    reachable: $REACHABLE_COUNT/3
...
if [ -n "$AUTO_SWITCHED_FROM" ]; then
    echo "bootstrap: profile=$PROFILE (auto-switched from $AUTO_SWITCHED_FROM, reachable=$REACHABLE_COUNT/3)" >&2
else
    echo "bootstrap: profile=$PROFILE  prom=$PROM_S  qdb=$QDB_S  api=$API_S" >&2
fi
...
#   2. Only fires when prom + qdb + api are ALL OFFLINE
...
if [ "$PROFILE" = "local" ] \
    ...
    echo "bootstrap: all 3 local services OFFLINE — firing 'docker compose up -d' in background (log: $AUTO_UP_LOG; opt out via TICKVAULT_NO_AUTO_UP=1)" >&2
