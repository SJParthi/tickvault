...
# if at least this many of the probed endpoints are REACHABLE. If the
# configured profile falls below this AND a different profile meets it, we
# auto-switch. After #O5 (2026-05-30) the Prometheus probe was removed
# (container removed in #O3), so the probed set is QuestDB + app-api (2);
# 2-of-N (both reachable) is the minimum useful set for the MCP log tools.
AUTO_SWITCH_MIN_REACHABLE=2
...
# Probe a profile and echo `<reachable_count> <qdb_s> <api_s>`.
# Side effects: sets global vars QDB, API, LOGS_SRC, LOGS_DIR
# to the profile's configured values (caller consumes them).
# prometheus_url DELETED in #O5 (2026-05-30) — Prometheus container removed in #O3.
# alertmanager_url DELETED in #O5 (2026-05-30) — Alertmanager container removed in #O2.
score_profile() {
    local profile="$1"
...
if [ "$AUTO_SWITCH_ENABLED" = "1" ] && [ "$REACHABLE_COUNT" -lt "$AUTO_SWITCH_MIN_REACHABLE" ]; then
    ...
    BEST_QDB="$QDB" BEST_API="$API"
    BEST_LOGS_SRC="$LOGS_SRC" BEST_LOGS_DIR="$LOGS_DIR"
    BEST_QDB_S="$QDB_S" BEST_API_S="$API_S"
    ...
    if [ "$BEST_PROFILE" != "$PROFILE" ]; then
        ...
        QDB="$BEST_QDB" API="$BEST_API"
        LOGS_SRC="$BEST_LOGS_SRC" LOGS_DIR="$BEST_LOGS_DIR"
        QDB_S="$BEST_QDB_S" API_S="$BEST_API_S"
...
# switched_from: ${AUTO_SWITCHED_FROM:-none}    reachable: $REACHABLE_COUNT/2
export TICKVAULT_MCP_PROFILE="$PROFILE"
# TICKVAULT_PROMETHEUS_URL DELETED in #O5 (2026-05-30) — Prometheus container removed in #O3.
# TICKVAULT_ALERTMANAGER_URL DELETED in #O5 (2026-05-30) — Alertmanager container removed in #O2.
...
# TICKVAULT_PROM_STATUS DELETED in #O5 (2026-05-30) — Prometheus container removed in #O3.
# TICKVAULT_ALERT_STATUS DELETED in #O5 (2026-05-30) — Alertmanager container removed in #O2.
...
if [ -n "$AUTO_SWITCHED_FROM" ]; then
    echo "bootstrap: profile=$PROFILE (auto-switched from $AUTO_SWITCHED_FROM, reachable=$REACHABLE_COUNT/2)" >&2
else
    echo "bootstrap: profile=$PROFILE  qdb=$QDB_S  api=$API_S" >&2
fi
...
#   2. Only fires when qdb + api are ALL OFFLINE
#      (any service up → assume operator is mid-debug and don't disturb).
#      (Prometheus probe removed in #O5 — container removed in #O3.)
...
if [ "$PROFILE" = "local" ] \
    ...
    echo "bootstrap: all local services OFFLINE — firing 'docker compose up -d' in background (log: $AUTO_UP_LOG; opt out via TICKVAULT_NO_AUTO_UP=1)" >&2
