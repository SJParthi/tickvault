...
- [x] Gate the 4 `/api/debug/*` routes in `crates/api` behind the existing `require_bearer_auth` middleware, applied UNCONDITIONALLY via a dedicated `debug_routes` sub-router `route_layer` (NOT gated on `feed_toggle_public`). HONEST CONTRACT (corrected post-review): every real boot enables auth (SSM token, hard-fail fetch), so at runtime these routes ALWAYS demand the bearer token — the middleware's disabled-passthrough is reachable only for empty-token constructions (tests). The tickvault-logs MCP LOG tools are unaffected (local filesystem reads, never these routes).
...
- [x] New router-level integration test file proving the auth gate on all 4 debug paths.
  ...
  - Tests: test_debug_routes_401_without_token, test_debug_routes_401_with_wrong_token, test_debug_routes_pass_auth_with_correct_token, test_debug_routes_pass_when_auth_disabled, test_debug_routes_401_on_head_without_token, test_health_stays_public_without_auth
...
- [x] Post-adversarial-review hardening (3-agent pass, 2026-07-04): (a) doctor.sh section-3 debug probe treats 401 as PASS ("gated, expected") so a correctly-secured host never false-FAILs; (b) operator-portal Lambda cross-verify probe authenticates — box-side SSM read of /tickvault/prod/api/bearer-token + Authorization header (token resolved at run time on the box, never in the SSM command document text); (c) lib.rs comment + this plan corrected — the passthrough premise holds only for empty-token constructions, never a real boot; (d) MCP tool_tickvault_api gains env-only TICKVAULT_API_BEARER_TOKEN header support; (e) HEAD-method gate test; (f) runbook Security notes + installer output + config/claude-mcp-endpoints.toml synced to the bearer-gated / 3001-only / logs-local reality.
  - Files: scripts/tv-tunnel/doctor.sh, deploy/aws/lambda/operator-control/handler.py, crates/api/src/lib.rs, scripts/mcp-servers/tickvault-logs/server.py, crates/api/tests/debug_auth_gate.rs, docs/runbooks/claude-mcp-access.md, scripts/tv-tunnel/install-mac.sh, config/claude-mcp-endpoints.toml
  - Tests: test_debug_routes_401_on_head_without_token; python3 -m unittest test_handler (169 tests) green; bash -n of the assembled cross-verify command; config guard crates/common/tests/claude_mcp_endpoints_config_guard.rs stays green

- [x] Post-approval addition (2026-07-04 dated operator quote — verbatim: "whicghever is recommended go ahea dudde okay?", given in response to the recommendation to bearer-gate the publicly-funnelled tokenless feed toggle): gate the MUTATING `POST /api/feeds/{feed}` behind `require_bearer_auth` UNCONDITIONALLY, in ALL modes. The 2026-06-23 tokenless-localhost dry-run carve-out (`feed_toggle_public=true` → public toggle) is RETIRED — with the 3001 funnel live it was a tokenless feed-disable DoS surface. Rule file updated FIRST per the PR-E lock protocol (websocket-connection-scope-lock.md "⚠ 2026-07-04 OPERATOR UPDATE — FEED TOGGLE BEARER-GATED IN ALL MODES" — includes the SSM token fetch + 0600 header-file curl example). The `feed_toggle_public` param is kept accepted-but-ignored (`_feed_toggle_public`) — removing it cascades across 11 call sites; both main.rs call-site comments + the /feeds page docstring/placeholder are synced (the page already sends `Authorization: Bearer` from sessionStorage — no UI change needed). Read-only GET /api/feeds + /api/feeds/health stay public (the "public read, authed toggle" READ half of the 2026-06-23 ruling is unchanged).
  - Files: .claude/rules/project/websocket-connection-scope-lock.md, crates/api/src/lib.rs, crates/api/src/handlers/feeds_page.rs, crates/api/tests/debug_auth_gate.rs, crates/app/src/main.rs
  - Tests: test_feeds_post_requires_auth_401_without_token_in_both_modes (flipped from the retired dry-run-public 200 ratchet), test_feeds_post_with_valid_token_not_401_in_both_modes (gate opens for the real token in both modes); debug_auth_gate.rs both-modes loops stay green (flag now ignored)
...
2. **Debug-route auth gating in `crates/api/src/lib.rs::build_router_with_auth`.** The 4 read-only debug routes (`GET /api/debug/logs/summary`, `GET /api/debug/logs/jsonl/latest`, `GET /api/debug/spill/status`, `GET /api/debug/cross-verify/latest`) move OFF `public_routes` into a dedicated `debug_routes: Router<SharedAppState>` which gets `.route_layer(axum::middleware::from_fn_with_state(auth_config.clone(), require_bearer_auth))` applied ALWAYS — deliberately NOT conditioned on `feed_toggle_public`. `require_bearer_auth` (crates/api/src/middleware.rs) passes through when `config.enabled == false` — but (corrected post-review, 2026-07-04) that state is reachable ONLY for empty-token constructions (unit/router tests): BOTH main.rs boot paths construct `ApiAuthConfig::from_token(<SSM token>)` with a hard-fail fetch, so on every real boot (prod AND local) auth is enabled and the debug routes demand `Authorization: Bearer <token>`, returning 401 otherwise — fail-closed by design, including on localhost. Route paths and handler bindings stay byte-identical; only their router placement changes. Final router assembly becomes `public_routes.merge(debug_routes).merge(protected_routes)` with the existing tracing + CORS layers unchanged.

MCP contract: the tickvault-logs LOG tools (`tail_errors`, `summary_snapshot`, …) read log files locally (never `/api/debug/*` over HTTP) — unaffected. `tool_tickvault_api` DOES 401 on `/api/debug/*` without a token even against localhost (auth is always enabled on a real boot); post-review it sends `Authorization: Bearer <TICKVAULT_API_BEARER_TOKEN>` when that env var is set (env-only — the token never enters the committed endpoints TOML).

## Edge Cases

- **Auth disabled (empty-token construction — tests only):** middleware passthrough — all 4 debug routes behave as before gating (200/404 depending on filesystem state). This keeps `build_router(state, &[], true)`-style router tests green. It is NOT a real-boot state: every real boot hard-fails without the SSM token, so runtime auth is always enabled.
...
- **MCP `tool_tickvault_api` calls to `/api/debug/*`:** 401 without a token (auth is always enabled on a real boot, including localhost). Mitigation (post-review): the tool sends `Authorization: Bearer <TICKVAULT_API_BEARER_TOKEN>` when the env var is set. The MCP LOG tools never call `/api/debug/*` over HTTP (filesystem reads) — unaffected either way.
...
- The tunnel exposure reduction itself is the security win: the auth-less QuestDB raw-SQL surface (port 9000) and three retired-service ports leave the public internet entirely. `scripts/tv-tunnel/doctor.sh` probes only 3001 and (post-review fix) treats 401 as PASS in BOTH probe sections — the generic port probe (`^(200|204|301|302|401|403)$`) AND the section-3 debug-endpoint probe (401 = "gated, expected"; 200/404 = auth-disabled construction) — so the doctor remains truthful on a correctly-secured host.
- The operator-portal Cross-verify card keeps rendering: its box-side probe now reads the bearer token from SSM at run time and sends the Authorization header; an unfetchable token degrades to the same truthful blank card as a stopped box (never a fabricated PASS, audit Rule 11).
