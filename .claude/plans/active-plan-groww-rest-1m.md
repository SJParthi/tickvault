...
| Chain-specific: expiry-day rollover | PR-3 | PR-3 | current expiry from the instruments CSV holds through the session (never-roll) | PR-3 | PR-3 |
| Chain-specific: chain-ok-but-spot-failed | PR-3 — the chain is NOT hard-gated on spot success (watch signal published unconditionally + fallback timer) | PR-3 | PR-3 | PR-3 | PR-3 |
| Contract-specific: zero-trade minutes on thin strikes | PR-4 — absent vs gap-filled candles is Unknown; missing contract minutes counted + reported, never fabricated | PR-4 | PR-4 | PR-4 | PR-4 |
