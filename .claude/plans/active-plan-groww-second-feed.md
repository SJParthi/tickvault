...
- [ ] PR-BV (bridge tick validation — security follow-up) — **the Groww bridge writes `feed='groww'` rows into the SHARED `ticks`/`candles_1m` tables; it MUST validate NDJSON fields from the sidecar before persist** (deep security review 2026-06-19, MEDIUM). Reject/clamp: non-finite or non-positive `ltp` (NaN/Inf/≤0), negative `cum_volume`, negative or far-future `ts_ist_nanos`/`exchange_ts_millis` (outside a sane IST window), negative `security_id`. Mirror the Dhan path's `is_valid_ltp` / `check_tick_ohlc_integrity` / NaN-clamp guards (phase-0-architecture.md Pillar 2) so a misbehaving sidecar cannot corrupt shared-table data + the 15:31 cross-verify. Also: bounded per-poll read (cap bytes) + `error!` (not `warn!`) on sustained sidecar read failure for operator visibility.
  - Files: `crates/app/src/groww_bridge.rs`, possibly a small validation helper in `crates/core/src/feed/groww/`
  - Tests: `test_groww_bridge_rejects_non_finite_ltp`, `test_groww_bridge_rejects_negative_volume`, `test_groww_bridge_rejects_far_future_ts`, `test_groww_bridge_bounded_read`
