...
- [ ] PR-B (Groww universe → ~779 subscribed) — **Rust builds the watch-list** (operator decision 2026-06-20): download Groww master `instrument.csv` (`growwapi-assets.groww.in/instruments/instrument.csv`) + the NIFTY-Total-Market list, join NTM ISINs → Groww `exchange_token` (cash stocks) + NSE indices → `exchange_token`, write a watch file the sidecar reads. Sidecar: read watch file (replace hardcoded WATCH), `subscribe_ltp` (stocks) + `subscribe_index_value` (indices, fixes the IDX_I MEDIUM). ~779 instruments.
  ...
  - [x] PR-B2i — INDICES end-to-end: Rust `extract_index_entries` now also includes the single BSE SENSEX (token `1`, groww_symbol `BSE-SENSEX`) alongside every NSE IDX row, and each index `WatchEntry` carries a Groww-native stable `security_id` (`stable_index_security_id` = FNV-1a of `groww_symbol`, top-bit-cleared → deterministic, positive, no collision with small numeric stock tokens; operator decision 2026-06-21 "Groww-native stable IDs"). `WatchEntry` gained `pub security_id: i64` (stocks = numeric token; indices = stable id) so the sidecar never re-derives identity. Dedup key widened to `(exchange, exchange_token, segment)` (BSE SENSEX `1` can't collide with an NSE stock `1`). Sidecar now subscribes BOTH `subscribe_ltp` (stocks) + `subscribe_index_value` (indices), emitting index ticks with segment `IDX_I`, ltp=`value`, cum_volume=0 (Option A), security_id looked up from the watch entry. Files: `crates/core/src/feed/groww/instruments.rs`, `scripts/groww-sidecar/groww_sidecar.py`. Tests: `test_extract_index_entries_includes_bse_sensex`, `test_stable_index_security_id_is_deterministic_positive_and_distinct`, `test_resolve_stock_entries_join_and_unresolved` (security_id), `test_watch_entry_serializes_to_sidecar_contract` (security_id) + 19 existing (23/23 green).


- [x] PR-C1 (one-click self-provision — operator 2026-06-21 "from Mac I will just click Run TickVault and internally it should take care of it, even inside AWS, especially the python installation + deps") — the Groww sidecar supervisor now **auto-discovers the host Python interpreter** (`discover_system_python` → pure `pick_system_python` over `PYTHON_CANDIDATES`) across every place a normal Mac/AWS host keeps it (Xcode CLT `/usr/bin/python3`, Homebrew `/opt/homebrew` + `/usr/local`, versioned `python3.x`, plain `python3`), instead of hardcoding `"python3"`. Combined with the already-automated venv-create + `pip install` (deps) + spawn + supervise, flipping Groww on and starting the app self-provisions **everything** with zero manual commands on Mac AND AWS (common runtime). Honest scope: we DISCOVER, never auto-INSTALL the OS interpreter (`brew` BANNED + sudo needed); every normal host already ships one. Files: `crates/app/src/groww_sidecar_supervisor.rs`. Tests: `test_pick_system_python_*` (4) + `test_python_candidates_cover_mac_and_aws_locations`.
