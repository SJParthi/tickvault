...
**Merged:** PR #1262 (`c4f8ba8a` on main) — archived 2026-06-29 per plan-enforcement.md Phase 4.
...
- [x] **HIGH-1 — trading-day-aware market-closed gate.** The `/feeds` health
...
- [x] **HIGH-2 — non-tick recovery edge for `auth_rejected`.** Today the ONLY
...
- [x] **MEDIUM (root) — generic `Error` class must NOT set `auth_rejected`.** The
...
apply to every item in this plan. Honest envelope: all three fixes are confined
to the **status-display / feed-health verdict layer** + the sidecar stderr
drain — cold-path only. They add no hot-path allocation and touch no
tick / dedup / persist / order path, so the DHAT zero-alloc and Criterion p99
rows are `N/A — cold-path display fix`. Coverage is proven by the pure-function
unit tests named per item (`feed_health.rs`, `market_hours.rs`,
`groww_sidecar_supervisor.rs`); composite-key uniqueness, DEDUP, and the
zero-tick-loss ring→spill→DLQ chain are unchanged. The honest "100%" claim is
100% inside the tested verdict envelope, with ratcheted regression coverage by
those unit tests.
