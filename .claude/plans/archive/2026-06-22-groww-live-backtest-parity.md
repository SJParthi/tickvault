...
> **⚠ HALTED 2026-06-26 — operator directive: Groww is LIVE-FEED-ONLY; never pull Groww historical/backtest data. The live-vs-backtest parity track (SP6/SP6a/SP6b/SP7) is CANCELLED until the operator re-authorizes.**
>
> **Verbatim operator quotes (2026-06-26, preserve exactly):**
...
>
> **Effect:** The remaining backtest-parity sub-PRs (SP5 parity audit table, SP6/SP6a/SP6b `BacktestSource` trait + Dhan/Groww fetchers, SP7 parity orchestrator) are CANCELLED. The Groww live-feed pipeline (live ticks → WAL → ring → spill → DLQ → 1m/21-TF candles tagged `feed='groww'`) is UNCHANGED and continues. NO Groww historical/backtest fetch may be implemented until the operator re-authorizes. **Dhan is unaffected** — Dhan's own historical cross-verify (`cross_verify_1m_boot.rs`) and prev-day OHLCV (Dhan, fail-soft) stay exactly as-is. This banner SUPERSEDES the live↔backtest deliverable in the APPROVED line below. The abandoned SP6a branch (`claude/parity-sp6-backtest-source` — partial `BacktestSource` trait + `DhanBacktestSource` relocation) was discarded; this file is retained as audit trail.
> See `.claude/rules/project/groww-second-feed-scope-2026-06-19.md` §33 for the binding rule.

**Status:** APPROVED (parity track HALTED 2026-06-26 — see banner above)
**Date:** 2026-06-22
...
- [CANCELLED 2026-06-26] SP5 — (was) ONE `feed_parity_1m_audit` table + writer (feed in DEDUP), merging the two audit modules. CANCELLED with the parity track per the operator LIVE-FEED-ONLY directive (banner above). The audit table existed solely to record live-vs-backtest parity mismatches; with no Groww backtest fetch, there is no Groww parity to audit. Dhan's existing `cross_verify_1m_audit` (Dhan-only) stays untouched.
- [CANCELLED 2026-06-26] SP6 / SP6a / SP6b — (was) `trait BacktestSource` + Dhan REST impl (moved from app) + Groww sidecar impl. CANCELLED: Groww historical/backtest pulls are FORBIDDEN until re-authorized. The partial SP6a branch (`BacktestSource` trait + `DhanBacktestSource` relocation, untracked files `crates/core/src/feed/backtest_source.rs`, `crates/core/src/feed/dhan/intraday.rs`, `crates/core/src/feed/groww/backtest.rs`, `scripts/groww-sidecar/groww_backtest_fetch.py`) was DISCARDED 2026-06-26.
- [CANCELLED 2026-06-26] SP7 — (was) generic parity orchestrator `run_parity_1m(feed, BacktestSource)` + boot wiring + generic `PARITY-01/02` error codes + rule file. CANCELLED with the parity track. The Groww/Dhan live pipelines remain siloed-but-live; no parity orchestrator is wired.
