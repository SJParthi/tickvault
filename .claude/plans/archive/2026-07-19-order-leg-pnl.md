...
**Two emission seams in `crates/app/src/order_runtime.rs`.** A `Copy` struct `LegPnlEvent { ts_utc_ns: i64, sid: u64, segment_code: u8, event_kind: LegPnlKind /* Mark|Fill */, net_lots: i32, lot_size: u32, avg_entry_price: f64, mark_price: f64, realized_pnl: f64, unrealized_pnl: f64 }` is emitted: `ts_utc_ns` is stamped AT THE EMIT SEAM from the process clock (`chrono::Utc::now().timestamp_nanos_opt().unwrap_or(0)`): neither `MarkUpdate` (order_runtime.rs:135-142) nor the fill event carries a timestamp field, so the seam is the ts source (round-5 fold).
1. after `apply_fill` → `risk.record_fill` (order_runtime.rs ~:821) — one `fill` row per applied paper fill;
2. after `risk.update_market_price` inside `process_mark` (order_runtime.rs ~:1210; fn def engine.rs:285) — one `mark` row per applied option mark on a leg with an open position or pending order (`process_mark` already filters, ~:1205-1209). Pending-only guard (round-5 fold): the mark seam is reachable with `has_pending_paper == true` and NO position (order_runtime.rs:1204-1208 gates on `has_position || has_pending_paper`); there `risk.position(sid)` returns `None` (engine.rs:424) and the seam emits NO row — early skip before any `LegPnlEvent` is built, never a zero/sentinel row. Pinned by `test_leg_pnl_mark_pending_no_position_no_row`.
...
2. The ErrorCode::all() count-assert bump (164 -> 165, error_code.rs:2771, dated comment) is now a NAMED mandatory edit in Item 6.
...
### Round-5 fold (2026-07-19)

Round-5 reviewer pair (fold verifier + fresh refuter) returned FIX-FIRST; folded here:
1. Count-assert citation corrected `error_code.rs:2772` -> `error_code.rs:2771` at both plan citations (2771 is the assert line; 2772 is the test fn's closing brace).
2. MAJOR — pending-only mark path: the mark seam fires on `has_position || has_pending_paper` (order_runtime.rs:1204-1208); with a resting paper order and NO position, `risk.position(sid)` is `None` (engine.rs:424) and the event has no P&L source. Resolution: EARLY-SKIP — no row, never a sentinel row; new Edge Case + `test_leg_pnl_mark_pending_no_position_no_row` pin.
3. MINOR — `ts_utc_ns` source named: stamped at the emit seam from the process clock; `MarkUpdate`/fill events carry no ts field.
4. MINOR — consumer flush cadence stated: the `order_update_events_boot` exemplar cadence (append per drained event, flush per drain batch; flush failure -> `discard_pending` + counter).
...
- Mark on a PENDING-ONLY leg (paper order resting, NO position): `risk.position(sid).is_none()` at the mark seam -> early-skip, NO row emitted (round-5 fold, MAJOR) — pinned by `test_leg_pnl_mark_pending_no_position_no_row`.
