...
**CADENCE-05:** new `ErrorCode::Cadence05RecoveryDegraded` ("CADENCE-05", Medium, auto-triage-safe YES, log-sink-only — NO Telegram, the Dhan noise-lock 4-item family is untouched) — fires once per lane/minute when the bounded native retries exhaust and the slot resolves via cross-fill. Registered per the full checklist: enum after `Cadence03SchedulerDegraded,` (~error_code.rs:1248), code_str arm (~:1473), severity arm (~:1790 region), runbook_path (~:1812 -> cadence-error-codes.md), auto-triage (~:2066), catalog + variant-count bump (~:2364), unit tests mirroring the c1/c2/c3 tests (~:2822-2837), crossref literal `CADENCE-05` + verbatim `Cadence05RecoveryDegraded` in cadence-error-codes.md.
...
- CADENCE-05 drift: crossref + tag + catalog-count guards fail the build on any unregistered surface.
...
- CADENCE-05 coded `error!` (log-sink-only, Medium) once per lane/minute on retry exhaustion -> cross-fill; the coalesced CADENCE-01 wrap-up gains the resolution token + retry_attempts fields.
