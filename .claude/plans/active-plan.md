...
- **Box auto-stopped during the smoke gate** — the gate verifies the EC2 state
  ...
  the not-running skip is 000-ONLY — a FAST 503/504 with the box not running
  at BOTH samples is, by that same physics, unambiguously a console-LAMBDA-
  layer fault (front/back wiring, lambda permission, back-lambda crash — the
  front maps back_not_configured/back_invoke_failed/back_function_error to a
  fast 503) that is fully probeable regardless of box state, and now FAILS
  CLOSED (the old arm exit-0-skipped it with a message asserting the code is
  impossible — laundering exactly the regression classes a terraform apply
  itself ships, on every off-window night/weekend apply, the gate's dominant
  execution regime; pinned by gate-matrix scenarios P + Q). HONEST COST
  (disclosed): a transient lambda-invoke throttle producing a fast 503 now
...
- **Future QuestDB renames `/index.html`** — HONEST ENVELOPE (fixer round 1,
  ...
  QDB-SHELL-CANARY-OK with no WARN/annotation; the code-checked canary WARNs
  on any non-200 and carries the observed code in the WARN line.
...
- **Dead box** — no static shell exists that could fake a 200 on a dead box.
  ...
  accepts ONLY 000 as the not-running window: a fast 503/504 with the box
  not running at both samples FAILS CLOSED as a console-lambda-layer fault
  (a not-running box physically cannot produce it through the 5s-capped
  curl).
...
- **FILE 5 gate behavioral matrix (round 8; CI-wired round 9; 18 scenarios
  since round 11):**
  ...
  "gate-matrix: all 18 scenarios green" (verbatim per-scenario table in the
