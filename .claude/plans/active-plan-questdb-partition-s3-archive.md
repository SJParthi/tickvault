...
## Review Round 2 Fix (2026-07-13, refuter round 2 on PR #1504 — 1 MEDIUM)

| Sev | Finding | Resolution |
|---|---|---|
| MEDIUM | The F1a reuse arm treated "existing S3 length == local gzip length" as content identity — a stale object from a failed prior run + a mutated partition (backfill into a >hot-window partition) + a compressed-length coincidence → verify passes → DROP with the S3 sole-copy missing the backfilled rows | **Content identity via SHA-256 additional checksums** (ETag deliberately NOT used — under SSE-KMS it is not the MD5, and the bucket's encryption config is unverifiable from the repo): (1) the gzip stream is SHA-256-hashed AS WRITTEN (`Sha256Writer` adapter; digest covers exactly the on-disk/uploaded bytes); (2) the conditional PutObject carries `.checksum_sha256(<base64>)` — S3 validates the body server-side and stores the attribute; (3) HeadObject uses `ChecksumMode::Enabled` and the reuse arm requires length match (fast pre-filter) AND stored-checksum == fresh digest — mismatch OR a checksum-less foreign/legacy object → `S3Conflict` (keep, loud, never overwrite); (4) `partition_archive_audit` gains a `gzip_sha256` STRING column (added to the CREATE — nothing deployed yet) and `VerifiedArchive` carries `gzip_sha256_hex`; (5) tests: PutObject-carries-checksum assertion in the full-flow stub test, reuse test now seeds (len, correct sha) and is renamed identity-based, 2 NEW stub tests (same length + different checksum → conflict-keep-no-drop; same length + missing checksum → conflict-keep), base64/hex known-vector tests (RFC 4648 + sha256("abc")), `Sha256Writer` streamed-vs-oneshot digest test; F4 guard extended to pin `head_object_meta(` + the checksum comparison in the reuse arm. Dep: `sha2` (already a workspace pin used by the app crate); base64 hand-rolled for the fixed 32-byte digest (known-vector-pinned) rather than adding a new supply-chain root. |
