...
- [x] **2c — `crates/tickvault-logs-mcp` rewrite + guard amendments + .mcp.json swap** (14-tool parity + harness + self-test; ci.yml matrix entry `tickvault-logs-mcp`; coverage floor measured-then-ratcheted + lockdown amend; validate-automation re-point; server.py + test_placeholder_fallback.py deleted after parity evidence)
