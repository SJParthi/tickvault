...
- [x] **Item 11** — Dual-gate market-hours fix (G1 exchange_ts + G2 wall-clock 60s grace) — `crates/core/src/pipeline/tick_processor.rs::is_wall_clock_within_persist_window` now uses upper bound `TICK_PERSIST_END_SECS_OF_DAY_IST + WS_GRACE_AFTER_CLOSE_SECS_U32` (= 15:31:00 IST) so a tick stamped 15:29:59.586 IST with `received_at` wall-clock 15:30:00.100 IST is ACCEPTED (was REJECTED pre-fix — the 2026-05-13 skipped-tick bug). G1 (`is_within_persist_window`, exchange_timestamp-based) unchanged. New constant `WS_GRACE_AFTER_CLOSE_SECS_U32 = 60` (compile-time-asserted equal to `WS_GRACE_AFTER_CLOSE_SECS`). 4 ratchet tests in `tick_processor.rs::tests`: `test_tick_with_exchange_ts_15_29_59_586_accepted_even_if_local_recv_15_30_00_100`, `test_tick_with_exchange_ts_15_30_00_000_rejected`, `test_ws_socket_stays_open_until_15_31_00`, `test_market_gate_does_not_call_local_now` (source-scan).
- [x] **Item 12** — `last_tick_audit` + `LastTickAfterBoundary` Telegram + banned-pattern hook — table DDL + writer in `crates/storage/src/last_tick_audit_persistence.rs` (8 unit tests + 2 source-scan ratchets); boot DDL wired in `crates/app/src/main.rs:3080` via `ensure_last_tick_audit_table`; meta-guard `test_last_tick_audit_table_is_wired_into_boot_ddl` in `secret_manager.rs:1062` pins both DDL boot wiring + variant definition. **Production caller** for the writer wired in `crates/core/src/pipeline/tick_processor.rs::run_tick_processor` G1 reject site: any tick with `exchange_secs_of_day >= TICK_PERSIST_END_SECS_OF_DAY_IST` spawns an audit-row write + increments `tv_late_tick_after_boundary_total`. Three new helpers `format_trading_date_ist`, `format_bar_minute_ist`, `exchange_segment_label_static` (5 ratchet tests for boundary values + 8 known segment codes + unknown→"UNKNOWN" safety). Telegram variant `NotificationEvent::LastTickAfterBoundary { security_id, exchange_segment, exchange_ts_nanos_of_day, nanos_past_close }` defined at `events.rs:1233` (Severity::Info). **Banned-pattern hook** Category 11 added to `.claude/hooks/banned-pattern-scanner.sh`: rejects `is_within_market_hours[A-Za-z_]*\([^)]*now\(\)` pattern (the 2026-05-13 dual-gate root cause).
...
- [x] **Item 17** — SEBI 24h JWT daily renewal observability + `auth_renewal_audit` — table DDL + writer in `crates/storage/src/auth_renewal_audit_persistence.rs` (12 tests); boot DDL wired in `crates/app/src/main.rs:3059`; production caller `TokenManager::write_renewal_audit_row` in `crates/core/src/auth/token_manager.rs:1016` writes audit rows for all 3 outcomes (Success / Failed / CircuitBreakerHalt) across both scheduled + force-renewal paths (6 audit-row sites total — pinned by `test_force_renewal_paths_write_audit_rows`). **Phase 0 Item 17 itself** (this PR, 2026-05-18) wired the missing positive-ping Telegram + Prometheus counter: scheduled-renewal success branch now emits `NotificationEvent::TokenRenewed` (Severity::Low, previously defined but never emitted — Rule 13) + increments `tv_token_renewals_total{source="scheduled", result="success"}`. Failure + circuit-breaker-halt paths also increment the same counter with `result="failed"` / `result="circuit_breaker_halt"`. Ratchets: `test_scheduled_success_emits_token_renewed_telegram` + `test_tv_token_renewals_total_counter_present_for_all_outcomes` (source-scan, Rule 13). The original spec asked for a new variant `JwtRenewalCompleted`; reused the existing equivalent `TokenRenewed` variant per CLAUDE.md "no half-finished implementations / no redundant abstractions".
