...
**Examples (good):**
- "100% inside the tested envelope: the IST-midnight seal burst absorbed by the derived seal ring `AGGREGATOR_MAX_SLOTS × TF_COUNT` = 600,000 (constant `SEAL_BUFFER_CAPACITY`, ratcheted by `seal_ring.rs`) → NDJSON spill → DLQ"
...
| `scripts/coverage-gate.sh` | post-merge | ratcheted per-crate floors (68.3–99.4, target 100%) per crate |
