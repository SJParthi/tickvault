...
> `WAL_REINJECT_SEND_TIMEOUT_SECS` (30) in `crates/common/src/constants.rs`,
...
**Metrics:**
- `tv_ws_wal_reinject_aborted_total{reason}` — one increment per abort.
...
- `tv_ws_wal_reinject_chunks_total` — one increment per delivered 8,192-frame
  chunk (progress signal for a large replay).
...
- Any file containing `WS-REINJECT-01`, `WsReinject01`, `reinject_wal_frames`,
  `tv_ws_wal_reinject_aborted_total`, or `tv_ws_wal_reinject_chunks_total`
