...
| Session model | **1 active session per key** — a 2nd login is refused ("User Already Connected"). SSM named lock `instance-lock-truedata` acquired BEFORE any connect; refused → degrade (no TrueData this boot), page, never fight the peer with a reconnect storm |
...
| Aggregator | shared engine CODE, OWN instance; `FeedStrategy::Truedata`; the DORMANT tick→timeframe aggregator is REVIVED and fed by TrueData ticks — derives the live timeframe set below |
| Live timeframe set | **1s + 1m + 3m core** (Quote 3); 5m/15m and intermediate-second frames are DISCARDABLE (derived only if a backtested-proven strategy needs them — config-gated). Backtest wide, run narrow |
...
| Identity | `security_id` = the TrueData numeric symbol token (from the TD Symbol Master); raw TrueData symbol string persisted in `instrument_lifecycle.symbol_name` (feed='truedata') for reversibility. Cross-feed joins by ISIN / canonical index symbol / (underlying, expiry) — NEVER native id (I-P1-11 + futidx-4 §2) |
