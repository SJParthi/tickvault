...
| `crates/core/src/feed/groww/auth.rs` | Mint DELETED. `run_groww_auth_smoke_check` = SSM read + pure `validate_groww_token_shape`; empty/placeholder → feed page shows "minter has not populated the token" |
| `crates/app/src/groww_sidecar_supervisor.rs` | NO credential fetch, NO secret in the child env — injects only `GROWW_SSM_TOKEN_PARAM` (a path string). Requirements-fingerprint marker re-provisions the venv when deps change |
...
| `scripts/groww-sidecar/requirements.txt` | `boto3` pinned; the TOTP dependency removed with the mint path |
