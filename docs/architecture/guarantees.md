...
| `SEAL_BUFFER_CAPACITY = AGGREGATOR_MAX_SLOTS x TF_COUNT` = 25,000 x 9 = **225,000** at TF_COUNT=9 (was 600,000 at TF_COUNT=24 until the 2026-09-19 nine-frame collapse — read the constant, not this number) (absorbs the IST-midnight seal burst) — **CORRECTED 2026-08-29**, this said a literal 200,000, which is 3x too small and is the exact restate-the-number-instead-of-the-constant error CLAUDE.md warns about by name | `crates/trading/src/candles/seal_ring.rs` | `seal_ring.rs::test_seal_buffer_capacity_absorbs_the_whole_midnight_burst` |
...
|---|---|---|
| Ratcheted per-crate LINE-coverage floors (app 72.6 · core 91.6 · storage 90.1 · trading 96.9 · api 98.6 · common 99.4 · default 63.0; floors only move up, 100% is the target — LINE coverage only, no branch gate) | `quality/crate-coverage-thresholds.toml` | `scripts/coverage-gate.sh`, post-merge only (not a PR blocker) |
...
| Ratcheted per-crate LINE-coverage floors (72.6–99.4, target 100%; no branch gate) | YES | post-merge only (not a PR blocker) |
...
| `percentage_change` + `open_percentage_change` candle columns wired end-to-end, and the removed `open_gap_pct` / `close_pct_from_prev_day` / `net_volume` absent from every link (2026-09-22: this row named the pre-2026-09-19 column names and two tests that no longer exist) | `crates/storage/src/{shadow_persistence,shadow_candle_writer,shadow_seal_columns}.rs` | `crates/storage/tests/candle_pct_column_guard.rs::{percentage_change,open_percentage_change}_column_wired_end_to_end` + `::{open_gap_pct,close_pct_from_prev_day}_is_gone_from_the_wire` + `::net_volume_is_gone_from_every_link_of_the_chain` |
