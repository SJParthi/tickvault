...
>
> **⚠ §5 RE-SUPERSEDED → r8g.xlarge + MULTI-AZ 2026-08-08 (operator Quote 13 in [`.claude/rules/project/daily-universe-scope-expansion-2026-05-27.md`](../../.claude/rules/project/daily-universe-scope-expansion-2026-05-27.md) §0/§7):** instance UPGRADED t4g.large → **r8g.xlarge** (Graviton4, **4 vCPU / 32 GiB**, memory-optimized) and — the load-bearing half — the **single-AZ subnet pin REMOVED** (subnets now in ap-south-1a/b/c, zone chosen by `var.availability_zone`). The 2026-08-07 type-only flip above FAILED with the SAME `InsufficientInstanceCapacity` and was rolled back, proving the constraint was the ZONE, not the size; `describe-instance-type-offerings` (live 2026-08-08) shows all 7 candidate types offered in all 3 AZs. Sized for the 13-timeframe (1s/5s/10s/15s/30s + 1m/2m/3m/5m/15m/30m/60m + 1d) current-day workload WITH raw-tick retention at ~25,000 instruments. EBS fresh-provision 20 → **100 GB**; budget kill-ceiling $35 → **$100**. Bill → **~₹5,824–₹7,382/mo** incl GST at ~210 hrs — the Quote 9 sub-₹1,000 target is **BREACHED ~6× and knowingly accepted**. SCOPE HONESTY: this sizes a box that CAN hold the workload; no live tick feed exists yet (Dhan + Groww live WS both retired, GDF/TrueData default-OFF), so filling it to 25,000 instruments needs a separate feed trial + its own dated scope. The current effective instance lock lives in that file's §7.

> **⚠ §5 RE-SUPERSEDED AGAIN → t4g.large 2026-08-07 (operator Quote 12 in [`.claude/rules/project/daily-universe-scope-expansion-2026-05-27.md`](../../.claude/rules/project/daily-universe-scope-expansion-2026-05-27.md) §0/§7):** instance UPGRADED t4g.medium → **t4g.large** (Graviton2, 2 vCPU / **8 GiB**, $0.0448/hr) purely to ESCAPE AN AWS CAPACITY OUTAGE — `InsufficientInstanceCapacity` for t4g.medium in ap-south-1a left the single-AZ-pinned box unstartable for whole trading sessions (Aug 5 = 0h, Aug 7 = 0h). Bill → ~₹1,896/mo at 270 hrs / ~₹1,473/mo at ~176 hrs incl GST; the Quote 9 sub-₹1,000 target is NOT met. 8 GiB also retires the §7 Rule 2 memory FLAG (the retained sizing formula predicted a ~2.5 GB app working set at ~770 SIDs, which does not fit 4 GiB). The current effective instance lock lives in that file's §7.
>
