...
### REL-10 — a CI runner reported a phantom "file does not exist on disk" for a source file that compiles cleanly in a sibling job of the SAME run
- **Severity:** Medium
- **Tag:** UNIVERSAL
- **Evidence:** PR #1199's docs-only commit (touching ONLY `docs/eap-feedback/tickvault-eap-dossier.md`, +19/-1) failed CI on `Test (common)`. The failing test was the repo's `runbook_cross_link_guard::every_repo_path_referenced_in_runbooks_resolves` meta-guard, which scans `docs/runbooks/*.md` and asserts every referenced repo path resolves on disk. It panicked claiming `docs/runbooks/cross-verify-1m.md` references `crates/storage/src/cross_verify_1m_audit_persistence.rs` — "does not exist on disk". But that source file DOES exist in the git tree at the exact failing commit (verified via the tree listing for the SHA) AND the `Test (storage)` job in the SAME workflow run compiled it successfully (it is wired into `crates/storage/src/lib.rs`). A file cannot simultaneously compile in the storage crate and be absent from the filesystem for the common crate at the same commit unless that one runner's working-tree checkout was partial/corrupt — a transient infra flake, plausibly induced by the same disk-pressure class as REL-04/REL-08. The misleading part: the failure surfaces as a deterministic-looking, code/docs-pointing assertion ("runbook references a path that doesn't exist"), strongly inviting a wrong "fix" (edit the doc or weaken the guard) when the doc and guard are both correct and the real cause is a flaky checkout. (`rerun-failed-jobs` via the GitHub MCP integration token returned 403 — "Resource not accessible by integration" — so the agent could not re-run the single failed job and had to re-trigger CI with a fresh commit instead.)
- **Suggested fix:** (a) Make runner checkout integrity verifiable — e.g. a post-checkout `git status --porcelain` / tree-hash assertion, or fail-fast if the working tree diverges from the checked-out SHA — so a partial checkout surfaces as an infra error, not a phantom test assertion. (b) Allow agents to re-run a single failed job (grant the integration the `actions:write` scope, or expose a re-run affordance) so a known-transient flake doesn't force a fresh no-op commit just to get a clean runner.
...
- **CI-flake finding (REL-10)** reproduces by comparing, for a single failing workflow run, the `runbook_cross_link_guard` panic in `Test (common)` (a referenced source path "does not exist on disk") against the SAME run's `Test (storage)` job successfully compiling that very file, and confirming the file is present in the git tree at the failing commit SHA — proving the "missing file" is a partial-checkout artifact on one runner, not a real broken link.
