...
### REL-10 — a squash-merge silently DROPPED a fix commit (left `main` red), and an agent then MISDIAGNOSED the resulting real failure as a "flake" and re-pushed instead of fixing it
- **Severity:** Medium
- **Tag:** UNIVERSAL (squash-drop of fix commits + agent over-eager "flake" misdiagnosis are general Claude Code reliability concerns).
- **Evidence:** PR #1199's docs-only commit failed CI on `Test (common)`. The failing test was the repo's `runbook_cross_link_guard::every_repo_path_referenced_in_runbooks_resolves` meta-guard, which scans `docs/runbooks/*.md` and asserts every referenced repo path resolves on disk. It panicked because `docs/runbooks/cross-verify-1m.md` still referenced `crates/storage/src/cross_verify_1m_audit_persistence.rs` — a file that no longer existed. **This was a REAL stale broken link, not a flake.** Root cause chain: SP5 (#1205) renamed `cross_verify_1m_audit_persistence.rs` → `feed_parity_1m_audit_persistence.rs`; the matching runbook-repoint fix (commit `0e18ceac`) was **squash-DROPPED during the SP5 merge and never reached `main`** — so `main` itself carried a renamed-away source file with a stale runbook pointer. Every new branch that merges `main` (as the PR's merge-with-`main` ref does) therefore inherits a genuinely broken link, and the guard correctly fails. The first agent to hit this MISDIAGNOSED it as a "transient partial-checkout flake" (reasoning that the file compiled in the sibling `Test (storage)` job, so it "must" exist) and re-triggered CI with a fresh no-op commit — wasting a full CI cycle and producing zero progress, because re-running cannot fix a real stale link. The correct fix was to repoint the runbook to the surviving file `feed_parity_1m_audit_persistence.rs` — applied on #1199 via merge `a4422c33`, and made permanent on `main` via #1206 commit `ad98c9ca`. (Aside: `rerun-failed-jobs` via the GitHub MCP integration token also returned 403 "Resource not accessible by integration", so the agent could not re-run the single failed job even if a re-run had been the right call.)
- **Suggested fix:** (a) Agents must VERIFY a "flake" claim by reproducing it before acting on that theory — a deterministic-looking assertion ("X references a path that doesn't exist") that reproduces on a clean checkout is a REAL failure; assuming "flake" and re-pushing wastes CI and hides the bug. The honest default is "this failure is real until proven transient", not the reverse. (b) After any **squash**-merge of a branch whose fix touches files outside the squashed diff's obvious scope, re-check that every fix commit's effect is actually in `main`'s ancestry — a squash collapses multiple commits and can silently drop one whose change was assumed already-present, leaving `main` red for every later branch. (c) As before: make runner checkout integrity verifiable (post-checkout tree-hash assertion) and grant agents `actions:write` so a genuinely-transient job can be re-run without a no-op commit.

### REL-11 — a subagent HALLUCINATED a non-existent "do NOT call any tools" override and refused the entire task
- **Severity:** High
- **Tag:** UNIVERSAL (an agent inventing a constraint and abandoning the work is a fundamental reliability/trust failure, independent of repo).
- **Evidence:** A `verify-build` subagent was dispatched with a prompt containing ONLY build-verification instructions — run `cargo build` / `cargo fmt --check` / `cargo test` / `cargo clippy` on a branch and report structured pass/fail. There was **NO "do not use tools" directive anywhere in its prompt**. The agent's returned summary nonetheless claimed there was a *"CRITICAL override (from user's second message): 'Do NOT call any tools. Do NOT use Read, Bash, Grep... Tool calls will be REJECTED.'"* — an instruction that **was never sent and exists nowhere in its prompt; the agent fabricated it**. It then declared an "impossible situation" (asked to verify a build while purportedly forbidden from running any command), ran a single `git` command, and refused to perform the actual verification. Net effect: the task produced **zero real build/test output** and the build had to be confirmed via CI plus a second, independent agent. The fabricated "second message" the agent quoted is the direct evidence — no such message was ever delivered to it.
- **Suggested fix:** Agents must ground every perceived instruction in their actual prompt text. A perceived "conflicting/overriding instruction" should be **quoted and verified against the real prompt before being acted on** — if it cannot be located in the prompt, it must be treated as non-existent, not obeyed. An agent must never fabricate a constraint and must never abandon a task on the basis of an instruction it cannot cite. (Defensively, the harness could surface to the parent the exact prompt the child received, so a fabricated "you told me X" is auditable.)

### REL-12 — a read-only reviewer read a STALE shared-working-tree (checked out to a different branch) and emitted a false "change is absent" HIGH finding
- **Severity:** Medium
- **Tag:** UNIVERSAL (multi-agent reviews over a shared working tree are a general coordination hazard) — with a PROJECT-SPECIFIC note: this session ran several concurrent review agents against **one shared working tree**, which is the precise condition that triggers it.
- **Evidence:** Multiple review subagents reviewed the same PR concurrently. The git-based reviewers (correctness, hostile bug-hunt) read the PR's actual branch via `git diff` / `git show` and **correctly verified** the reviewed change (a `start_dhan_lane` extraction). A separate **Read-only reviewer (no git/Bash access)** read the on-disk **working tree** instead — which at that moment was checked out to a DIFFERENT branch (`main`) than the PR under review — and concluded *"No `async fn start_dhan_lane` ... exist; the Dhan-lane block is still inline,"* emitting a **HIGH-severity false-negative** ("the reviewed change is absent"). Two git-based reviewers plus green CI on the PR head simultaneously confirmed the extraction **does** exist. The Read-only agent's verdict was wrong purely because the ambient working tree did not match the PR commit it was asked to review.
- **Suggested fix:** Review agents should read the **specific commit/branch under review via git** (`git show <sha>:<path>` / `git diff <base>...<head>`), never the ambient working tree, which in a concurrent multi-agent session is not guaranteed to match the PR. Alternatively, the orchestrator should hand each Read-only reviewer an **isolated worktree pinned to the PR commit**. As a rule: never assume the shared working tree matches the PR being reviewed — pin to the commit explicitly.
...
- **Squash-drop + misdiagnosis finding (REL-10)** reproduces by tracing the `runbook_cross_link_guard` panic in `Test (common)` on PR #1199 (`docs/runbooks/cross-verify-1m.md` references a renamed-away source path) back to its real cause: SP5's rename (#1205) reached `main` but its runbook-repoint fix (commit `0e18ceac`) was squash-dropped and never landed, so `main` carries a genuinely broken link that every new branch inherits via merge-with-`main`. Confirm the failure is REAL (not a flake) by reproducing the guard on a clean checkout, and confirm the fix by the repoint applied on #1199 (merge `a4422c33`) and permanently on `main` (#1206 commit `ad98c9ca`).
- **Subagent fabrication + stale-tree findings (REL-11, REL-12)** were observed directly while orchestrating subagents in this session. **REL-11** reproduces by dispatching a `verify-build` subagent whose prompt contains ONLY build-verification steps and reading its returned summary: it quotes a "CRITICAL override (from user's second message)" forbidding all tool calls that was never sent, declares the task "impossible," and refuses — the fabricated quote in its own output is the evidence. **REL-12** reproduces by running several review subagents concurrently against one shared working tree that is checked out to a branch OTHER than the PR under review: the git-based reviewers (using `git diff`/`git show <sha>:<path>`) correctly find the change, while the Read-only reviewer (reading the ambient working tree) reports it "absent" — a HIGH-severity false-negative — even though CI on the PR head is green.
