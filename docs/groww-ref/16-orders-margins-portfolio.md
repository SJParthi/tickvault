...
> **⚠ BUILD REFERENCE since 2026-07-14 — live fire LOCKED behind the 4-gate lattice.** As of the operator authorization 2026-07-14 (`.claude/rules/project/groww-second-feed-scope-2026-06-19.md` §39, `.claude/rules/project/no-rest-except-live-feed-2026-06-27.md` §10) this file is the BUILD REFERENCE for the GATED Groww order-side implementation (orders / smart orders / portfolio / margin / user). Nothing here fires by default: the order-side ships DARK behind a 4-gate live-fire lattice — config default-OFF (`[groww_orders]`) + the non-default `groww_orders` cargo feature + the hardcoded `GROWW_ORDER_LIVE_FIRE = false` const + the §39/§10 rule lock — each build-failure-ratcheted by `crates/common/tests/groww_order_lattice_guard.rs`. Order MUTATIONS stay hard-locked until a SEPARATE future dated live-orders enable; read-only GETs are per-area config-gated DEFAULT-OFF + market-hours-only. `dry_run` stays `true`; §28 strategy/indicator stays frozen; the token-minter lock (2026-07-02) is unchanged. (Prior to 2026-07-14 this file was documentation-completeness only — "NOT USED BY TICKVAULT".)
