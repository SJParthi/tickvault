...
| ------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| RequestCode   *required*                    | int         | Code for subscribing to particular data mode. Refer to [feed request code](./18-annexure.md#feed-request-code) to subscribe to required data mode |
| InstrumentCount   *required*                | int         | No. of instruments to subscribe from this request                                                                                          |
| InstrumentList.ExchangeSegment   *required* | enum string | Exchange Segment of instrument to be subscribed as found in [Annexure](./18-annexure.md#exchange-segment)                                   |
| InstrumentList.SecurityId   *required*      | string      | Exchange standard ID for each scrip. Refer [Instrument List](./21-instruments-scrip-master.md)                                              |

Feed Request Codes (see [Annexure](./18-annexure.md#feed-request-code)): `15` = Ticker subscription, `17` = Quote subscription, `21` = Full subscription, `12` = Disconnect.
...
All responses from Dhan Market Feed consists of [Response Header](#response-header) and Payload. Header for every response message remains the same with different [feed response code](./18-annexure.md#feed-response-code), while the payload can be different.
...
| ----- | -------- | ---- | --------------------------------------------------------------------------------------- |
| `1`   | [ ] byte | `1`  | Feed Response Code can be referred in [Annexure](./18-annexure.md#feed-response-code)   |
| `2-3` | int16    | `2`  | Message Length of the entire payload packet                                             |
| `4`   | [ ] byte | `1`  | Exchange Segment can be referred in [Annexure](./18-annexure.md#exchange-segment)       |
| `5-8` | int32    | `4`  | Security ID - can be found in [Instrument List](./21-instruments-scrip-master.md)       |
...
| Bytes   | Type      | Size | Description                                                                                                |
| ------- | --------- | ---- | ------------------------------------------------------------------------------------------------------------ |
| `0-8`   | [ ] array | `8`  | [Response Header](#response-header) with code `2`    Refer to [enum](./18-annexure.md#feed-response-code)   |
...
| Bytes   | Type      | Size | Description                                                                                                 |
| ------- | --------- | ---- | ------------------------------------------------------------------------------------------------------------- |
| `0-8`   | [ ] array | `8`  | [Response Header](#response-header) with code `6`    Refer to [enum](./18-annexure.md#feed-response-code)    |
...
| Bytes   | Type      | Size | Description                                                                                                |
| ------- | --------- | ---- | ------------------------------------------------------------------------------------------------------------ |
| `0-8`   | [ ] array | `8`  | [Response Header](#response-header) with code `4`    Refer to [enum](./18-annexure.md#feed-response-code)   |
...
| Bytes  | Type      | Size | Description                                                                                                |
| ------ | --------- | ---- | ------------------------------------------------------------------------------------------------------------ |
| `0-8`  | [ ] array | `8`  | [Response Header](#response-header) with code `5`    Refer to [enum](./18-annexure.md#feed-response-code)   |
...
| Bytes    | Type                   | Size  | Description                                                                                                |
| -------- | ---------------------- | ----- | ------------------------------------------------------------------------------------------------------------ |
| `0-8`    | [ ] array              | `8`   | [Response Header](#response-header) with code `8`    Refer to [enum](./18-annexure.md#feed-response-code)   |
| `9-12`   | float32                | `4`   | Latest Traded Price of the subscribed instrument                                                             |
| `13-14`  | int16                  | `2`   | Last Traded Quantity                                                                                         |
| `15-18`  | int32                  | `4`   | Last Trade Time (LTT) - EPOCH                                                                                |
| `19-22`  | float32                | `4`   | Average Trade Price (ATP)                                                                                    |
...
| Bytes  | Type      | Size | Description                                                                                                 |
| ------ | --------- | ---- | ------------------------------------------------------------------------------------------------------------- |
| `0-8`  | [ ] array | `8`  | [Response Header](#response-header) with code `50`    Refer to [enum](./18-annexure.md#feed-response-code)   |
| `9-10` | int16     | `2`  | Disconnection message code - see [Annexure → Data API Errors](./18-annexure.md#data-api-error)               |

You can find detailed Disconnection message code description in [Annexure → Data API Errors](./18-annexure.md#data-api-error).
