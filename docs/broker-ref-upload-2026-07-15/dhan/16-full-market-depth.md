# Full Market Depth (20 & 200 Level, WebSocket, Binary)
...
Similar to [Live Market Feed](./15-live-market-feed.md), all request messages over WebSocket are in JSON whereas all response messages over WebSocket are in Binary.
...
| ------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| RequestCode   *required*                    | int         | Code for subscribing to particular data mode. `23` for Full Market Depth. Refer to [feed request code](./18-annexure.md#feed-request-code)        |
...
| ---------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| RequestCode   *required*     | int         | Code for subscribing to particular data mode. `23` for Full Market Depth. Refer to [feed request code](./18-annexure.md#feed-request-code)        |
| ExchangeSegment   *required* | enum string | Exchange Segment of instrument to be subscribed as found in [Annexure](./18-annexure.md#exchange-segment)                                         |
...
All responses from Dhan Market Feed consists of Response Header and Payload. Header for every response message remains the same with different [feed response code](./18-annexure.md#feed-response-code), while the payload can be different.
...
| `3`    | [ ] byte | `1`  | Feed Response Code can be referred in [Annexure](./18-annexure.md#feed-response-code)   |
...
| Bytes    | Type                    | Size  | Description                                                                                                                                     |
| -------- | ----------------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `0-12`   | [ ] array               | `12`  | [Response Header](#response-header)    `41` for Bid Data (Buy)     `51` for Ask Data (Sell)     Refer to [enum](./18-annexure.md#feed-response-code) |
...
| Bytes     | Type                    | Size   | Description                                                                                                                                     |
| --------- | ----------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `0-12`    | [ ] array               | `12`   | [Response Header](#response-header-1)    `41` for Bid Data (Buy)     `51` for Ask Data (Sell)     Refer to [enum](./18-annexure.md#feed-response-code) |
...
| Bytes   | Type      | Size | Description                                                                                                 |
| ------- | --------- | ---- | ------------------------------------------------------------------------------------------------------------- |
| `0-12`  | [ ] array | `8`  | Response Header with code `50`    Refer to [enum](./18-annexure.md#feed-response-code)                       |
| `13-14` | int16     | `2`  | Disconnection message code - see [Annexure → Data API Errors](./18-annexure.md#data-api-error)               |
