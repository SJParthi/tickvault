# Annexure (Enums, Error Codes, Feed Codes)
...
(The string attribute is used in REST/JSON requests; the numeric enum is used in the binary feed Response Header's Exchange Segment byte.)
...
(`MTF` — Margin Traded Fund — also appears as a product type across Orders/Super Order/Forever Order APIs.)
...
(For 20/200-level Full Market Depth: `41` = Bid Data (Buy), `51` = Ask Data (Sell) — see [Full Market Depth](./16-full-market-depth.md).)
...
Error response body structure (see [Introduction → Errors](./01-introduction.md#errors)):
...
(Also used as WebSocket feed disconnection message codes.)
