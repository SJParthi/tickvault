...
Platforms can receive order updates originating for all users connected to their platform/app for which [Partner Login](./02-authentication.md#for-partners) module needs to be used.
...
| OrderType         | enum string | Order Type   `LMT` for Limit   `MKT` for Market   `SL` for Stop Loss   `SLM` for Stop Loss Market           |
...
| Instrument        | string      | Instrument in which order is placed - [Annexure → Instrument](./18-annexure.md#instrument)                  |
| Symbol            | string      | Symbol in which order is placed - Refer [Instrument List](./21-instruments-scrip-master.md)                 |
| ProductName       | string      | Product type of the order placed - [Annexure → Product Type](./18-annexure.md#product-type)                 |
...
| DisplayName       | string      | Name of instrument in which order is placed - Refer [Instrument List](./21-instruments-scrip-master.md)     |
...
| Remarks           | string      | `Super Order` if the order is part of [super order](./04-super-order.md)                                    |
