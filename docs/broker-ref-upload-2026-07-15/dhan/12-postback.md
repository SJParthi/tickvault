...
| omsErrorCode        | string      | Error code in case the order is rejected or failed *(spelled "omserroeCode" in source table)*     |
...
> Note: To receive Postback originating for all orders placed from a platform/app, [Partner Login](./02-authentication.md#for-partners) module needs to be used.
