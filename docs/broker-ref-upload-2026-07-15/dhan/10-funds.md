...
**Curl Request** (as shown verbatim on the source page — note the URL contains stray escaped spaces `%20%20`; the effective endpoint is `https://api.dhan.co/v2/margincalculator/multi`)

```
curl --request POST \
  --url https://api.dhan.co/v2/%20%20/margincalculator/multi \
  --header 'Accept: application/json' \
  --header 'Content-Type: application/json' \
  --header 'access-token: ' \
  --data '{
...
| availabelBalance    | float  | Available amount to trade (note: field name is misspelled "availabel" in the actual API response) |
