...
| securityId   *required*      | string       | Underlying exchange standard ID for each scrip. Refer [Instrument List](./21-instruments-scrip-master.md)                           |
| instrument      *required*   | enum string  | Instrument type of the scrip. Refer [Annexure → Instrument](./18-annexure.md#instrument)                                            |
| expiryCode   *required*      | enum integer | Expiry of the instruments. Refer [Instrument List](./21-instruments-scrip-master.md)                                                |
...
(`iv`, `oi`, `strike`, `spot` arrays are returned when requested via `requiredData`. `ce` holds CALL data, `pe` holds PUT data.)
