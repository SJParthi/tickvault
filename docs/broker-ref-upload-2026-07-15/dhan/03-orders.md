...
**Parameters** (same field set applies to Order Book, Get Order by Order Id, and Get Order by Correlation Id)
...
| exchangeSegment     | enum string | Exchange Segment of instrument to be subscribed as found in [Annexure](./18-annexure.md#exchange-segment)       |
...
**Parameters**: identical to the Order Book parameters table above.
...
**Response Structure**: identical to Get Order by Order Id response above (single order object).
...
**Parameters**: identical to the Trade Book parameters table above.
