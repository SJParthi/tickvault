...
Caller-side guards (automatic, no manual action needed):
  - Application persistence marker prevents same-day double writes (Layer 1)
  - DEDUP verification gate blocks writes if DEDUP not confirmed (Layer 2)
  - QuestDB DEDUP UPSERT KEYS as last line of defense (Layer 3)
  - Safe to call persist_instrument_snapshot() multiple times — idempotent
```
...
### Deduplication & Idempotency (7-Layer Enforcement)

```
✅ 7 layers of enforcement prevent duplicate instrument data:

Layer 1 (Application Persistence Marker):
  - instrument-persist-date.txt written ONLY after successful QuestDB write
  - Checked BEFORE any QuestDB write — blocks same-day double writes
  - Separate from build freshness marker (build can succeed, persist can fail)

Layer 2 (DEDUP Verification Gate):
  - verify_dedup_active() enables AND verifies DEDUP on all 5 tables
  - If verification FAILS → write is BLOCKED (prevents duplicates)
  - No more "best-effort" — hard gate before every write

Layer 3 (QuestDB DEDUP UPSERT KEYS):
  - instrument_build_metadata: UPSERT KEYS(timestamp, csv_source)
  - fno_underlyings: UPSERT KEYS(timestamp, underlying_symbol)
  - derivative_contracts: UPSERT KEYS(timestamp, security_id)
  - subscribed_indices: UPSERT KEYS(timestamp, security_id)
  - instrument_lifecycle: UPSERT KEYS(snapshot_date, security_id, event_type)

Layer 4 (Delta Detection):
  - Day-over-day comparison detects added/expired/modified contracts
  - Lifecycle events written to instrument_lifecycle table

Layer 5 (instrument_lifecycle Table):
  - Audit trail of every change (contract_added, contract_expired, lot_size_changed, etc.)

Layer 6 (Build-Time Validation):
  - Check 10: derivative → instrument_info cross-reference
  - Check 11: subscribed index count = expected 31

Layer 7 (Pre-Commit Hooks):
  - dedup-latency-scanner.sh blocks O(n) operations on hot path
  - data-integrity-guard.sh blocks price data manipulation
```
...
### Block 01.2 — Day-Over-Day Lifecycle Tracking

When the instrument universe is rebuilt daily, the delta detector compares
today's universe with yesterday's (loaded from rkyv binary cache) and produces
lifecycle events for every change.

**Table 5: `instrument_lifecycle`** — records contract_added, contract_expired,
lot_size_changed, tick_size_changed, underlying_added, underlying_removed events.

**Module:** `crates/core/src/instrument/delta_detector.rs`

**Full documentation:** See `docs/dhan-ref/09-instrument-master.md` (Dhan API ground truth) and
`docs/architecture/instrument-technical-design.md` for technical design covering all edge cases.

### Implementation Files

| # | File | Purpose |
|---|------|---------|
| 1 | `crates/storage/src/instrument_persistence.rs` | QuestDB ILP writer for all 5 tables + persistence marker + DEDUP gate |
