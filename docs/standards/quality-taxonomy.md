...
| 1.3 | Property-based (proptest) | Randomized invariant verification | Malformed ticks, edge floats, instrument counts 0..25000 | `ENFORCED` | 9 proptest! blocks across 6 crates, enforced by `scripts/test-coverage-guard.sh` type 6 | `22-test-types.md` type 6 |
...
| 1.6 | Snapshot/golden tests | Output comparison against stored baselines | WS subscription format, ILP format, JSON API response stability | `ENFORCED` | Snapshot tests exist in core crate, enforced by `scripts/test-coverage-guard.sh` type 8 | `22-test-types.md` type 8 |
| 1.7 | Concurrency tests (loom) | Exhaustive interleaving verification | ArcSwap token renewal, crossbeam channels, papaya map | `ENFORCED` | loom tests in `crates/trading/tests/loom_circuit_breaker.rs`, enforced by `scripts/test-coverage-guard.sh` type 15 | `22-test-types.md` type 15 |
| 1.8 | Benchmark tests (criterion) | Statistical latency measurement | <10ns parse, <100ns OMS, <10μs pipeline | `CONFIGURED` | criterion benches exist in `crates/core/benches/`, enforced by `scripts/test-coverage-guard.sh` workspace check | `22-test-types.md` (workspace category) |
...
**Why:** Every failure scenario in `docs/standards/failure-scenarios.md` (30+ scenarios) must be tested: WS disconnect mid-tick, QuestDB write failure, OOM, etc.
