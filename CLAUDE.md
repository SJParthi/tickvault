...
**Logs (tracing → CloudWatch Logs via CW agent in prod; Loki/Alloy local only):** Structured JSON. The 8 filtered codes (see `deploy/aws/terraform/error-code-alarms.tf`) page via CloudWatch metric-filter alarms → SNS → Telegram; all other coded ERRORs are log-sink-only unless they have their own metric alarm or typed NotificationEvent.
