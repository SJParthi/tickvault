...
| `crates/app/src/spot_1m_rest_boot.rs::PersistTracker::committed` + `crates/app/src/groww_spot_1m_boot.rs` twin | **ADDED 2026-08-21 — recorded as NOT a risk, which is the point of the row.** A 2026-08-21 sweep reported both as "UNBOUNDED — no cap, no eviction, no reset". True of the TYPE and misleading about the risk: the key is `SecurityId`, one entry per SID, max-merged — and the only SIDs either leg ever fetches are the **4 hardcoded `SPOT_1M_REST_INDICES`**. Real ceiling ≈ 4 entries, ~64 bytes. Capping it would be theatre. Listed because this table's failures have run in BOTH directions — a stale row once manufactured a false finding (`day_ohlc_tracker`, 2026-08-12) — and "we checked and it is fine" is worth as much shelf space as "we checked and it is not". |
