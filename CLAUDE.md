...
| `crates/core/src/pipeline/tick_gap_detector.rs::scan_silence` | **O(n)** in tracked instruments. Inherent: "which instruments are silent?" is a question about all of them at once. Zero-alloc. *(**CORRECTED 2026-08-10:** this row called it a "cold-path **sweep**", which implies it runs. It has **ZERO production callers** — every `scan_silence` reference in `crates/*/src` is inside its own file's test module. It is dead code, and describing dead code as a running sweep is the false-OK class rule 11 forbids. Either wire it or delete it; until then it is listed here as DEAD, not as a sweep.)* |
| `crates/trading/src/candles/multi_tf_aggregator.rs` slot growth | a single growth step is **O(n)** and UNBOUNDED (the `Vec` copies every existing ~5.4 KB slot). *(**CORRECTED 2026-08-10:** this row claimed the growth was "Eliminated by pre-sizing with `with_capacity` at boot". **There is no boot call site** — no `MultiTfAggregator::new/default/with_capacity` exists anywhere in `crates/*/src` outside test modules, so the stated mitigation does not exist and the whole aggregator is unreachable in today's REST-only runtime. A mitigation that is documented but not implemented is worse than a documented gap, because it stops anyone from looking. The growth is UNMITIGATED and the code is DORMANT.)* |
